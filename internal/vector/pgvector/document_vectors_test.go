...
func TestDocumentVectorGenerationDimensionMigrationPostgres(t *testing.T) {
	t.Run("backfills authority and restores enforcement", func(t *testing.T) {
		db := openPGTestDB(t)
		require.NoError(t, Migrate(t.Context(), db, 0, false))
		_, err := db.Exec(`DROP TABLE document_vector_backend_generations CASCADE`)
		require.NoError(t, err)
		_, err = db.Exec(`
			INSERT INTO document_vector_embeddings (token, generation_id, dimension, embedding)
			VALUES ('legacy-token', 701, 3, '[1,0,0]'::vector)`)
		require.NoError(t, err)

		require.NoError(t, Migrate(t.Context(), db, 0, true))
		var dimension int
		require.NoError(t, db.QueryRow(`
			SELECT dimension FROM document_vector_backend_generations WHERE generation_id = 701`).Scan(&dimension))
		assert.Equal(t, 3, dimension)
		_, err = db.Exec(`
			INSERT INTO document_vector_embeddings (token, generation_id, dimension, embedding)
			VALUES ('wrong-dimension', 701, 4, '[1,0,0,0]'::vector)`)
		require.Error(t, err, "validated authority FK rejects later mixed-dimension rows")
	})

	t.Run("rejects conflicting legacy dimensions", func(t *testing.T) {
		db := openPGTestDB(t)
		require.NoError(t, Migrate(t.Context(), db, 0, false))
		_, err := db.Exec(`DROP TABLE document_vector_backend_generations CASCADE`)
		require.NoError(t, err)
		_, err = db.Exec(`
			INSERT INTO document_vector_embeddings (token, generation_id, dimension, embedding)
			VALUES
				('legacy-three', 702, 3, '[1,0,0]'::vector),
				('legacy-four', 702, 4, '[1,0,0,0]'::vector)`)
		require.NoError(t, err)

		err = Migrate(t.Context(), db, 0, true)
		require.ErrorContains(t, err, "document vector generation contains multiple dimensions")
		var rows int
		require.NoError(t, db.QueryRow(`
			SELECT COUNT(*) FROM document_vector_embeddings WHERE generation_id = 702`).Scan(&rows))
		assert.Equal(t, 2, rows, "failed migration preserves the legacy rows for repair")
	})
}

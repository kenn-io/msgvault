...
func (b *Backend) readyAccelerator(
	...
	return b.readyAcceleratorQuery(ctx, b.db, generationID, dimension)
}
...
func (b *Backend) readyAcceleratorQuery(
	...
	err := queryer.QueryRowContext(ctx, `
		SELECT a.generation_id, a.state, a.table_name, a.dimension,
		  ...
		  FROM vector_accelerators a
		  JOIN index_generations g ON g.id = a.generation_id
	...
	if errors.Is(err, sql.ErrNoRows) {
		return AcceleratorStatus{}, false, nil
	}
	if err != nil {
		return AcceleratorStatus{}, false, fmt.Errorf("lookup ready accelerator: %w", err)
	}
	if a.TableName != acceleratorTableName(generationID) ||
		a.Vec1Version != vec1.Version || a.Vec1Version != b.vec1Version {
		return AcceleratorStatus{}, false, nil
	}
	exists, err := acceleratorTableExists(ctx, queryer, a.TableName)
	if err != nil {
...
func acceleratorTableExists(
	...
	var exists int
	err := queryer.QueryRowContext(ctx, `SELECT 1 FROM sqlite_master
...
func (b *Backend) readyAcceleratorForWrite(
	...
	if errors.Is(err, sql.ErrNoRows) {
		//nolint:nilnil // No matching ready accelerator is the normal write fallback.
		return nil, nil
	}
	if err != nil {
		return nil, fmt.Errorf("lookup ready accelerator for write: %w", err)
	}
	if a.TableName != acceleratorTableName(generationID) ||
		a.Vec1Version != vec1.Version || a.Vec1Version != b.vec1Version {
		//nolint:nilnil // Invalid metadata disables write-through acceleration.
		return nil, nil
	}
	exists, err := acceleratorTableExists(ctx, tx, a.TableName)
	if err != nil {
		return nil, fmt.Errorf("check accelerator table for write: %w", err)
	}
	if !exists {
		//nolint:nilnil // A missing physical table disables write-through acceleration.
		return nil, nil
	}
	return &a, nil
}

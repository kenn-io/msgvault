...
func (b *Backend) SearchWithMetadata(
	...
	if k <= 0 {
		return []vector.Hit{}, vector.SearchMetadata{}, nil
	}
	var dimension int
	...
	if b.acceleratorMode == "exact" {
		...
		return hits, exactSearchMetadata(hits, k, "exact"), err
	}
	...
	if err != nil {
		...
		return b.exactSearchFallback(ctx, gen, queryVec, k, filter)
	}
	defer func() { _ = tx.Rollback() }()
	accelerator, ready, err := b.readyAcceleratorQuery(ctx, tx, gen, dimension)
	if err != nil {
		_ = tx.Rollback()
		if ctx.Err() != nil {
			return nil, vector.SearchMetadata{}, ctx.Err()
	...
	if !filter.IsEmpty() {
		...
		if len(messageIDs) <= threshold && fitsExact {
			...
			if err == nil {
				...
				return hits, vector.SearchMetadata{
					PoolSaturated: len(hits) >= k, CandidateCount: len(messageIDs), Accelerator: "exact-filter",
				}, nil
			}
			if ctx.Err() != nil {
				return nil, vector.SearchMetadata{}, ctx.Err()
...
func (b *Backend) exactSearchFallback(
	ctx context.Context,
	gen vector.GenerationID,
	queryVec []float32,
	k int,
	...
	hits, err := b.searchExact(ctx, gen, queryVec, k, filter)
	return hits, exactSearchMetadata(hits, k, "exact-fallback"), err
}
...
func (b *Backend) searchAccelerated(
	...
	for {
		...
		metadata := vector.SearchMetadata{
			CandidateCount: len(rowIDs), Accelerator: acceleratorKind,
		}
		if len(hits) >= k {

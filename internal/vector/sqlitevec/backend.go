...
type Options struct {
	...
	// selects the production default; it is primarily exposed for tests until
	// the public accelerator tuning settings are wired.
	ANNWorkCeiling  int
	ANNOversample   int
	ANNNProbe       int
...
func (b *Backend) Upsert(ctx context.Context, gen vector.GenerationID, chunks []vector.Chunk) error {
	...
	// next.
	// A fresh scan-and-fill batch has no rows to replace. Avoid asking vec0
	// to evaluate a partition delete in that overwhelmingly common case: the
	// cost grows with the existing partition and made large initial builds
	// progressively slower despite every requested message being new.
	if preexisting > 0 {
...
func (b *Backend) readyAcceleratorsForOrphanPrune(
	...
	rows, err := tx.QueryContext(ctx, `
		SELECT a.generation_id, a.state, a.table_name, a.dimension,
		  ...
		  FROM vec.vector_accelerators a
		  JOIN vec.index_generations g ON g.id = a.generation_id
		 WHERE a.kind = ? AND a.state = 'ready'
	...
	if err != nil {
		return nil, fmt.Errorf("list ready accelerators for orphan prune: %w", err)
	}
	defer func() { _ = rows.Close() }()
	var accelerators []AcceleratorStatus
	for rows.Next() {
		var accelerator AcceleratorStatus
		if err := rows.Scan(
			&accelerator.GenerationID, &accelerator.State, &accelerator.TableName,
			&accelerator.Dimension, &accelerator.IndexedCount, &accelerator.LastEmbeddingID,
			&accelerator.SourceRevision, &accelerator.ModelConfig, &accelerator.Vec1Version,
		); err != nil {
			return nil, fmt.Errorf("scan ready accelerator for orphan prune: %w", err)
		}
		if accelerator.TableName != acceleratorTableName(accelerator.GenerationID) ||
			accelerator.Vec1Version != b.vec1Version {
			continue
		}
		accelerators = append(accelerators, accelerator)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate ready accelerators for orphan prune: %w", err)
	}
	if err := rows.Close(); err != nil {
		return nil, fmt.Errorf("close ready accelerators for orphan prune: %w", err)
	}
	eligible := accelerators[:0]
	for _, accelerator := range accelerators {
		var exists int
		if err := tx.QueryRowContext(ctx, `SELECT 1 FROM vec.sqlite_master
			WHERE type = 'table' AND name = ?`, accelerator.TableName).Scan(&exists); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				continue
			}
			return nil, fmt.Errorf("check orphan-prune accelerator table: %w", err)
		}

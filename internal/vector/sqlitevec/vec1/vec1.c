...
/* To make AVX2 blocks more readable. */
  ...
  */
  ...
  for(ii=0; ii<nThread; ii++){
    ...
    ** process vectors from iFirst to (iEof-1), inclusive.  */
    ASSIGN_VECTORS_TO_JOB(pVec, pK, ii, nThread);
...
#if 0
  rc = vec1KMeansParallel(pVec, pQueue, nK, aCode);
#else
...
static void vec1TrainLogTimes(Vec1TrainCtx *p){
  ...
  if( p->zProfileFunction ){
    ...
    char *zSql = sqlite3_mprintf(
        "SELECT \"%w\"('"
        "{\"rot_codebooks\": %lld, \"rot_covariance\": %lld, "
        "\"rot_jacobi\": %lld, \"rot_vectors\": %lld, "
        "\"coarse_init\": %lld, \"coarse_kmeans\": %lld, "
        "\"residuals\": %lld, \"codebooks\": %lld}')",
        p->zProfileFunction,
        p->aTime[VEC1_TRAINING_ROT_CODEBOOKS],
        p->aTime[VEC1_TRAINING_ROT_COVARIANCE],
        p->aTime[VEC1_TRAINING_ROT_JACOBI],
        p->aTime[VEC1_TRAINING_ROT_VECTORS],
        p->aTime[VEC1_TRAINING_COARSE_INIT],
        p->aTime[VEC1_TRAINING_COARSE_KMEANS],
        p->aTime[VEC1_TRAINING_RESIDUALS],
        p->aTime[VEC1_TRAINING_CODEBOOKS]
    );
#endif

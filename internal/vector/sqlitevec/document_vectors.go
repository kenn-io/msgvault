...
func (b *DocumentBackend) Search(ctx context.Context, generationID document.GenerationID, dimension int, query []float32, k int) ([]document.Hit, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("check document vector table: %w", err)
	}
	if exists == 0 {
	...
	if err := b.db.QueryRowContext(ctx, `
		SELECT COUNT(*) FROM document_vector_embeddings
		WHERE generation_id = ? AND dimension = ?`, int64(generationID), dimension).Scan(&count); err != nil {
		return nil, fmt.Errorf("count document vectors: %w", err)
	}
	if count == 0 {
		return []document.Hit{}, nil
	}
	...
	q := fmt.Sprintf(`
		WITH knn AS MATERIALIZED (
			...
			WHERE generation_id = ? AND embedding MATCH ? AND k = ?
		)
		SELECT m.token, 1.0 - knn.distance AS score
		FROM knn
		JOIN document_vector_embeddings m
		  ON m.document_vector_id = knn.document_vector_id
	...
	hits := make([]document.Hit, 0, min(k, count))
	for rows.Next() {

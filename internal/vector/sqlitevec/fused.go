...
func (b *Backend) FusedSearch(ctx context.Context, req vector.FusedRequest) ([]vector.FusedHit, bool, error) {
	if req.QueryVec != nil && b.acceleratorMode != "exact" {
		var dimension int
		if err := b.db.QueryRowContext(ctx, `SELECT dimension FROM index_generations WHERE id = ?`,
			int64(req.Generation)).Scan(&dimension); err == nil && len(req.QueryVec) == dimension {
			_, ready, readyErr := b.readyAccelerator(ctx, req.Generation, dimension)
			if readyErr != nil && ctx.Err() != nil {
				return nil, false, ctx.Err()
			}
			if ready {
				vectorHits, metadata, err := b.SearchWithMetadata(
	...
	return b.fusedSearchExact(ctx, req)
}
...
func (b *Backend) fuseAcceleratedSignals(
	...
	return hits, bm25Saturated || vectorSaturated, nil
}

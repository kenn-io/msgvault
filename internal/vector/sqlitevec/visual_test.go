...
func TestVisualBackendPublishesOnlyCurrentLiveTokens(t *testing.T) {
	...
	require.NoError(t, f.Store.ConsentVisualGeneration(t.Context(), generation.ID))
	...
	require.NoError(t, f.Store.ActivateVisualGeneration(t.Context(), generation.ID, highWater))

...
const (
	...
	multimodalEligibilityPolicyVersion  = 1
...
func (m MultimodalConfig) validate() error {
	...
	if _, err := normalizedMultimodalEndpoint(m.Endpoint); err != nil {
	...
	if m.configuredAnimatedGIFs() && !multimodalAnimatedGIFDefault {
		return errors.New("vector.multimodal.include_animated_gifs: provider motion capability is not enabled in this build")
	}

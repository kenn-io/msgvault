...
func TestDiscordRepairAddsVoiceMetadataAndPreservesStoredMedia(t *testing.T) {
	...
	require.NoError(err)
	assert.Equal(`{"discord":{}}`, refs["discord:voice-1"].Metadata)
	assert.Equal(`{"discord":{"waveform":"%%%"}}`, refs["discord:voice-2"].Metadata)
	...
	require.NoError(err)
	assert.Equal(`{"old":4}`, invalidRefs["discord:invalid"].Metadata)

...
func notionDiscovery(raw jsontext.Value) (notionDiscoveryWire, bool) {
	var wire struct {
		MeetingNotes struct {
			Children struct {
...
func decodeNotionActions(fields map[string]jsontext.Value, discovery notionDiscoveryWire, discoveryOK bool) ([]Action, Coverage, string) {
	if !discoveryOK {
...
func notionTreeBlocks(raw jsontext.Value, expectedRootID string) ([]jsontext.Value, bool) {
	...
	traversal := notionTreeTraversal{
		pages:       tree.Pages,
		blocks:      []jsontext.Value{tree.Root},
		blockVisits: 1,
	}
	if !rootValid {
...
func notionTraversalPage(raw jsontext.Value) ([]jsontext.Value, bool, string, bool) {
...
func notionBlockTraversalHeader(raw jsontext.Value) (string, bool, bool) {
	var fields map[string]jsontext.Value
	if json.Unmarshal(raw, &fields) != nil || fields == nil {
		return "", false, false
	}
...
func notionResolvedUsers(fields map[string]jsontext.Value) map[string]notionResolvedUser {
	var users []struct {
		ID    string `json:"id"`
...
func notionAction(raw jsontext.Value, ordinal int) (Action, bool) {
	var fields map[string]jsontext.Value
	if json.Unmarshal(raw, &fields) != nil || fields == nil {
		return Action{}, false
	}
	...
	var todo map[string]jsontext.Value
...
func notionParticipants(fields map[string]jsontext.Value, discovery notionDiscoveryWire) []Participant {
	var labels []string

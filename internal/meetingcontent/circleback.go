...
func decodeCircleback(fields map[string]jsontext.Value) Content {
...
func decodeCirclebackActions(meeting map[string]jsontext.Value) ([]Action, Coverage, string) {
...
func circlebackAssignee(raw jsontext.Value) (string, string, bool) {
...
func decodeCirclebackTranscript(raw jsontext.Value) Transcript {
	if len(raw) == 0 {
...
func decodeCirclebackSegment(raw jsontext.Value) (Segment, bool, bool) {
	if isNull(raw) {
...
func decodeCirclebackStringAliases(fields map[string]jsontext.Value, keys ...string) Section {
	present := false
...
func circlebackOptionalString(raw jsontext.Value) (string, bool) {
	if len(raw) == 0 || isNull(raw) {
		return "", true
	}
...
func circlebackFirstValidString(fields map[string]jsontext.Value, keys ...string) (string, bool) {
	for _, key := range keys {
		raw, ok := fields[key]
		if !ok {
			continue

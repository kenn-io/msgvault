...
func TestGrantCoversAnyScopeKeepsDraftOperationsSeparate(t *testing.T) {
	tests := []struct {
		...
		{name: "readonly covers reads", granted: []string{ScopeGmailReadonly}, accepted: ScopesGmailDraftRead, want: true},
		...
		{name: "metadata does not cover draft", granted: []string{ScopeGmailMetadata}, accepted: ScopesGmailDraftRead, want: false},

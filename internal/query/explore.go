...
// buildExploreLogicalSQLNoLists is like buildExploreLogicalSQL but omits the
// participant_ids, participant_labels, and participant_domains list columns
// from logical_entries. Queries that join directly to message_recipients,
// messages.sender_id, and conversation_participants (instead of unnesting
// pre-built participant lists) use this to avoid the whole-archive list
// aggregation inside analytical_entries — the dominant source of DuckDB temp
// storage pressure on large mixed-source archives (see issue #523).

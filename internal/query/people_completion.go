...
type PeopleCompletion struct {
	...
	MatchValue                     string
	Source                         string
	CurrentRelationshipTemperature int       `json:"-"`
...
type PeopleCompleter interface {
	CompletePeople(ctx context.Context, request PeopleCompletionRequest) (*PeopleCompletionResponse, error)
}
...
func (e *DuckDBEngine) CompletePeople(
	...
	normalizedUsername := normalizePeopleCompletionUsernameText(queryText)
	digits := peopleCompletionDigits(queryText)
	directory := quoteIdentitySQLPath(e.parquetPath(identityindex.DatasetPeople))
	rows, err := e.db.QueryContext(ctx, `
		WITH params AS (
			SELECT ?::VARCHAR AS query_text, ?::VARCHAR AS query_username,
			       ?::VARCHAR AS query_digits
		), candidates AS (
			       ...
			       CASE
			           WHEN primitive.kind = 'phone' AND params.query_digits != '' THEN
			               CASE
			                   ...
			                   ELSE 2
			               END
			           WHEN primitive.kind = 'username' AND params.query_username != '' THEN
			               CASE
			                   WHEN primitive.match_value = params.query_username THEN 0
			                   WHEN starts_with(primitive.match_value, params.query_username) THEN 1
			                   ELSE 2
			               END
			...
			WHERE (primitive.kind = 'username' AND params.query_username != ''
			       AND contains(primitive.match_value, params.query_username))
			   OR (primitive.kind != 'username'
			       AND contains(primitive.match_value, params.query_text))
		...
		SELECT participant_id, display_label, kind, display_value, match_value, source,
		       current_temperature, last_at
		FROM deduplicated
		WHERE duplicate_rank = 1
		ORDER BY match_rank, kind_rank, current_temperature DESC, last_at DESC,
		         participant_id,
		         kind, lower(display_value), source
		LIMIT ?
	`, normalized, normalizedUsername, digits, limit)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&row.CurrentRelationshipTemperature, &row.LastAt,
		); err != nil {
...
func normalizePeopleCompletionUsernameText(value string) string {
	return strings.TrimLeft(NormalizePeopleCompletionText(value), "@")
}
...
func PeopleCompletionMatchRank(
	...
	if kind == PeopleCompletionUsername {
		queryText = strings.TrimLeft(queryText, "@")
...
func SortPeopleCompletions(rows []PeopleCompletion, queryText string) {
	slices.SortStableFunc(rows, func(a, b PeopleCompletion) int {
		...
		if order := cmp.Compare(b.CurrentRelationshipTemperature, a.CurrentRelationshipTemperature); order != 0 {
			return order
		}
		if order := b.LastAt.Compare(a.LastAt); order != 0 {
			return order
		}

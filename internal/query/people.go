...
// CTE) plus the parameter values it binds. person_entries always projects
// exactly the columns the aggregates in searchPeople consume: it is
// referenced more than once, so DuckDB materializes it, and carrying
// logical_entries.* would materialize every wide list/text column per row.
//
...
//     in the raw participant tables (message_recipients, messages.sender_id,
//     conversation_participants), filtered inside the analytical context.
//     The semi-join shape cannot be used because a conversation entry's
...
//
//     The fan-out joins logical_entries directly to message_recipients,
//     messages.sender_id, and conversation_participants (via a UNION of
//     three branches) instead of unnesting a pre-built participant_ids list.
//     This avoids the whole-archive list aggregation in analytical_entries
//     and the subsequent fan-out that exhausted DuckDB temp storage on large
//     mixed-source archives (see issue #523). The UNION correctly handles
//     non-chat entries (recipients + sender) vs. chat entries (conversation
//     participants), and deduplicates participants who appear in both
...
func personEntriesCTE(exactID *int64, memberIDs []int64, conditions, clustersGlob string) (string, []any) {
	...
	JOIN (
		SELECT le_inner.entry_key, mr.participant_id
		FROM logical_entries le_inner
		JOIN message_recipients mr ON mr.message_id = le_inner.anchor_message_id
		UNION
		SELECT le_inner.entry_key, m.sender_id
		FROM logical_entries le_inner
		JOIN messages m ON m.id = le_inner.anchor_message_id
		WHERE m.sender_id IS NOT NULL
		UNION
		SELECT le_inner.entry_key, cp.participant_id
		FROM logical_entries le_inner
		JOIN conversation_participants cp ON cp.conversation_id = le_inner.conversation_id
		WHERE le_inner.entry_kind = 'conversation'
	) AS ep ON ep.entry_key = le.entry_key
	JOIN canon cn ON cn.participant_id = ep.participant_id`, clustersGlob), nil
	}
	if len(memberIDs) == 0 {
	...
	if conditions != "true" {
		...
		// Repeat member IDs for each of the three EXISTS subqueries.
		for range 2 {
	...
	FROM logical_entries le
	WHERE EXISTS (SELECT 1 FROM message_recipients mr
	              WHERE mr.message_id = le.anchor_message_id
	              AND mr.participant_id IN (` + inList + `))
	   OR EXISTS (SELECT 1 FROM messages m
	              WHERE m.id = le.anchor_message_id
	              AND m.sender_id IN (` + inList + `))
	   OR EXISTS (SELECT 1 FROM conversation_participants cp
	              WHERE cp.conversation_id = le.conversation_id
	              AND cp.participant_id IN (` + inList + `))`, args
	}

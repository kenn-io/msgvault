...
func (e *DuckDBEngine) ExploreGroups(ctx context.Context, request ExploreGroupRequest) (*ExploreGroupResponse, error) {
	...
	logicalSQL := buildExploreLogicalSQL(conditions)
	if spec.noLists {
		logicalSQL = buildExploreLogicalSQLNoLists(conditions)
	}
	queryText := logicalSQL + spec.cte + `
), grouped AS (
...
type groupExpressions struct {
	...
	fromSuffix  string
	whereSuffix string
	// noLists requests buildExploreLogicalSQLNoLists instead of
	// buildExploreLogicalSQL, omitting participant list columns from
	// logical_entries. Used by the "participant" dimension, which joins
	// directly to message_recipients, messages.sender_id, and
	// conversation_participants instead of unnesting pre-built lists.
	noLists bool
}
...
// dimension groups by canonical identity-cluster IDs: raw participants are
// resolved through canon before grouping, and the DISTINCT mirrors
// personEntriesCTE/buildRelationshipsSQL — an entry listing several aliases of
// one person collapses to a single (entry, canonical) row, so the entry is
// never double-counted (entry_key is projected only to carry per-entry
// uniqueness through that DISTINCT).
//
// The "participant" dimension joins logical_entries directly to
// message_recipients, messages.sender_id, and conversation_participants
// (via a UNION of three branches) instead of unnesting a pre-built
// participant_ids list, and requests logical_entries without list columns
// (noLists). This avoids the whole-archive list aggregation in
// analytical_entries and the subsequent fan-out that exhausted DuckDB temp
// storage on large mixed-source archives (see issue #523).
func exploreGroupExpressions(dimension, clustersGlob string) (groupExpressions, error) {
	...
	JOIN canon cn ON cn.participant_id = ep.participant_id`,

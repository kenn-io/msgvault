...
func TestDuckDBTextSnapshotRevisionUsesCommittedCacheMarker(t *testing.T) {
	...
	require.NoError(err)
	require.NoError(os.WriteFile(CacheStatePath(analyticsDir), data, 0o600))

	rows, second, err := engine.ListConversationsSnapshot(t.Context(), filter)
	require.NoError(err)
	assert.Empty(rows)
	...
	require.NoError(err)
	assert.Equal(second, sameLogicalRequest)
}
...
func TestDuckDBTextSnapshotRevisionTracksHybridSQLiteTimeline(t *testing.T) {
	...
	rows, after, err := engine.ListConversationMessagesSnapshot(
		t.Context(), conversationID, scope.Filter,
	)
	require.NoError(err)
	require.Len(rows, 2)
	assert.Equal([]int64{801, 802}, []int64{rows[0].ID, rows[1].ID})
...
func TestDuckDBListConversationsParticipantMembership(t *testing.T) {
	...
	const (
		...
		senderFallbackConversationID  = int64(705)
		recipientFallbackConversation = int64(706)
		unrelatedFallbackConversation = int64(707)
	)
	...
	// Direct chats imported without conversation_participants must still be
	// discoverable through sender and recipient edges. Once an edge matches,
	// the filter keeps the complete conversation rather than only that message.
	b.AddMessage(MessageOpt{SourceID: sourceID, ConversationID: senderFallbackConversationID, MessageType: "imessage", SenderID: &aliceID})
	b.AddMessage(MessageOpt{SourceID: sourceID, ConversationID: senderFallbackConversationID, MessageType: "imessage", SenderID: &ownerID})

	recipientMatchMessageID := b.AddMessage(MessageOpt{SourceID: sourceID, ConversationID: recipientFallbackConversation, MessageType: "imessage", SenderID: &ownerID})
	b.AddTo(recipientMatchMessageID, aliceAliasID, "Alice Alias")
	b.AddMessage(MessageOpt{SourceID: sourceID, ConversationID: recipientFallbackConversation, MessageType: "imessage", SenderID: &ownerID})

	b.AddMessage(MessageOpt{SourceID: sourceID, ConversationID: unrelatedFallbackConversation, MessageType: "imessage", SenderID: &sameNameID})
	b.AddTo(b.AddMessage(MessageOpt{SourceID: sourceID, ConversationID: unrelatedFallbackConversation, MessageType: "imessage", SenderID: &ownerID}), sameNameID, "Alice Cluster")
	...
	assert.ElementsMatch([]int64{
		...
		ownerConversationID,
		senderFallbackConversationID,
		recipientFallbackConversation,
	}, conversationIDs)
	assert.Equal(int64(2), byID[senderFallbackConversationID].MessageCount)
	assert.Equal(int64(2), byID[recipientFallbackConversation].MessageCount)
}

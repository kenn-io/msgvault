...
//
// "interactions" joins logical_entries directly to message_recipients,
// messages.sender_id, and conversation_participants instead of unnesting a
// pre-built participant_ids list: the list aggregation in analytical_entries
// and the subsequent fan-out were the dominant sources of DuckDB temp storage
// pressure on large mixed-source archives (see issue #523).
//
...
func buildRelationshipsSQL(conditions, clustersGlob, ownersGlob string) string {
	...
	return buildExploreLogicalSQLNoLists(conditions) + fmt.Sprintf(`
), clusters AS (
    ...
    SELECT le.*,
        CASE WHEN le.entry_kind IN ('event','meeting') THEN
            EXISTS (SELECT 1 FROM message_recipients mr
                    JOIN owner_participant_ids opi ON opi.participant_id = mr.participant_id
                    WHERE mr.message_id = le.anchor_message_id)
            OR EXISTS (SELECT 1 FROM messages m
                       JOIN owner_participant_ids opi ON opi.participant_id = m.sender_id
                       WHERE m.id = le.anchor_message_id AND m.sender_id IS NOT NULL)
            OR EXISTS (SELECT 1 FROM conversation_participants cp
                       JOIN owner_participant_ids opi ON opi.participant_id = cp.participant_id
                       WHERE cp.conversation_id = le.conversation_id)
        ELSE FALSE END AS with_owner

...
func TestDuckDBParquetDeletionTargetsIncludeLegacyGmailSource(t *testing.T) {
	require := require.New(t)
	assert := assert.New(t)
	b := NewTestDataBuilder(t)
	...
	nonGmailSourceID := b.AddSourceWithType("other@example.invalid", "imap")
	aliceID := b.AddParticipant("alice@example.com", "example.com", "Alice")
	bobID := b.AddParticipant("bob@example.com", "example.com", "Bob")

	const subject = "Legacy Gmail deletion"
	addMessage := func(sourceID int64) int64 {
		messageID := b.AddMessage(MessageOpt{
			SourceID:    sourceID,
			Subject:     subject,
			MessageType: messageTypeEmail,
		...
		b.AddFrom(messageID, aliceID, "Alice")
		b.AddTo(messageID, bobID, "Bob")
		return messageID
	}

	explicitMessageID := addMessage(explicitSourceID)
	legacyMessageID := addMessage(legacySourceID)
	nonGmailMessageID := addMessage(nonGmailSourceID)
	b.AddMessage(MessageOpt{
		SourceID:         legacySourceID,
		Subject:          "Legacy chat",
		MessageType:      messageTypeIMessage,
	...
	var storedSourceType string
	err := engine.db.QueryRowContext(ctx,
		fmt.Sprintf("SELECT source_type FROM read_parquet('%s') WHERE id = ?", engine.parquetPath("sources")),
		legacySourceID,
	).Scan(&storedSourceType)
	require.NoError(err, "read legacy source fixture")
	assert.Empty(storedSourceType, "fixture must preserve the legacy empty source type")

	conversationRows, err := engine.ListConversations(ctx, TextFilter{})
	require.NoError(err, "list conversations")
	sourceTypes := make([]string, len(conversationRows))
	for i, row := range conversationRows {
		sourceTypes[i] = row.SourceType
	}
	assert.Contains(sourceTypes, "", "generic conversation queries must preserve the stored empty source type")
	...
	targets, err := engine.GetDeletionTargetsByFilter(ctx, MessageFilter{MessageType: messageTypeEmail})
	require.NoError(err, "filter")
	assertTargets("filter", targets)

	targets, err = engine.GetDeletionTargetsByMessageIDs(ctx,
		[]int64{explicitMessageID, legacyMessageID, nonGmailMessageID})

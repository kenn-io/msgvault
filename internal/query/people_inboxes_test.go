...
func TestPersonInboxesListsDeduplicatedChatSources(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(CacheStatePath(engine.analyticsDir), stateData, 0o600))

	result, err := engine.ListPersonInboxes(context.Background(), PersonInboxRequest{CanonicalID: subject})
	require.NoError(t, err)
	require.Len(t, result.Rows, 2)

	assert.Equal(t, state.Revision(), result.CacheRevision)
	assert.Equal(t, int64(9), result.IdentityRevision)
	assert.Equal(t, PersonInboxRow{

...
func testDocumentVectorOperationsState(t *testing.T) {
	...
	require.NoError(t, f.Store.InitSchema(), "the rebuild-index migration is replay-safe")
	...
	operations, err := f.Store.GetDocumentVectorOperationsStatus(t.Context(), generation.DocumentVectorGenerationSpec, egressFingerprint, 0, "", 10)
	require.NoError(t, err)
	...
	t.Run("configured target rotation builds beside the old active generation", func(t *testing.T) {
		...
		operations, err := rotatedFixture.Store.GetDocumentVectorOperationsStatus(t.Context(), desired, strings.Repeat("d", 64), 0, "", 10)
		require.NoError(err)
...
func TestDocumentVectorPublicationRecoverySchema(t *testing.T) {
	assert := assert.New(t)
	require := require.New(t)
	f := storetest.New(t)
	if f.Store.IsPostgreSQL() {
...
func TestDocumentVectorChunkCandidatesIncludeServedFallbackHead(t *testing.T) {
	require := require.New(t)
	f := storetest.New(t)
	profile, hash := seedDocumentPublicationAuthority(t, f)
	...
	require.NoError(err)
	require.Len(candidates, 1)
	assert.Equal(t, profile.ID, candidates[0].ExtractionProfileID)
}

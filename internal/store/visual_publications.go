...
func (s *Store) ListObsoleteVisualTokens(ctx context.Context, generationID int64, limit int) ([]string, error) {
	...
	rows, err := s.db.QueryContext(ctx, s.dialect.Rebind(`
		...
		ORDER BY message_id, blob_hash, media_input_key LIMIT ?`), generationID, limit)
	if err != nil {
...
func (s *Store) ListVisualGenerationTokens(ctx context.Context, generationID int64) ([]string, error) {
	rows, err := s.db.QueryContext(ctx, s.dialect.Rebind(`
		...
		WHERE generation_id = ? AND pending_vector_token IS NOT NULL`), generationID, generationID)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
func (s *Store) ClearObsoleteVisualToken(ctx context.Context, generationID int64, token string) error {
	_, err := s.db.ExecContext(ctx, s.dialect.Rebind(`
		...
		WHERE generation_id = ? AND state <> 'current' AND current_vector_token = ?`), generationID, token)
	return err
}
	...
	if generation.State == VisualGenerationRetired {
		if err := s.withTxContext(ctx, func(tx *loggedTx) error {
			...
			_, err := q.Exec(`UPDATE visual_generations
				SET state = 'building', source_fence = 0, consented_at = NULL, activated_at = NULL
...
func (s *Store) ConsentVisualGeneration(ctx context.Context, generationID int64) error {
	result, err := s.db.ExecContext(ctx, s.dialect.Rebind(`
		UPDATE visual_generations SET consented_at = `+s.dialect.Now()+`
...
func (s *Store) ClaimVisualWork(
	...
	err := s.db.QueryRowContext(ctx, `
		INSERT INTO visual_work_claims
			(generation_id, message_id, blob_hash, media_input_key,
			 proposed_revision, lease_owner, lease_expires_at, fencing_token, source_fence)
		VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?)
		...
		RETURNING lease_owner, lease_expires_at, fencing_token, source_fence
	`, request.GenerationID, request.Owner.MessageID, request.Owner.BlobHash,
		...
		expiresAt, request.SourceFence, request.Now).Scan(
...
func (s *Store) scanVisualClaim(
	...
	err := s.db.QueryRowContext(ctx, `
		SELECT lease_owner, lease_expires_at, fencing_token, source_fence

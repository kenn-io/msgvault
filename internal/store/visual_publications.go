...
func (s *Store) ListStaleVisualMessageIDs(ctx context.Context, generationID int64, limit int) ([]int64, error) {
	...
	// Oldest-touched first: a re-rejected retryable row gets a fresh
...
func (s *Store) PrepareVisualPublication(
	...
	err = s.withTxContext(ctx, func(tx *loggedTx) error {
		q := boundQuerier{ctx: ctx, q: tx}
		if err := verifyVisualGenerationLive(q, prepared.Claim.GenerationID); err != nil {
			return err
		}
...
func (s *Store) CommitVisualPublication(
	...
	err := s.withTxContext(ctx, func(tx *loggedTx) error {
		q := boundQuerier{ctx: ctx, q: tx}
		if err := verifyVisualGenerationLive(q, claim.GenerationID); err != nil {
			return err
		}
...
// verifyVisualGenerationLive refuses publication writes against a retired
// generation. Retirement marks the generation first and then enumerates
// tokens; without this fence a racing worker could publish after the
// enumeration and leave its vector behind in the backend.
func verifyVisualGenerationLive(q querier, generationID int64) error {
	...
	if err := q.QueryRow(`SELECT state FROM visual_generations WHERE id = ?`,
		...
		return fmt.Errorf("read visual generation state: %w", err)
	}

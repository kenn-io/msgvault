...
func (s *Store) CompletePersonProfilesContext(
	...
	queryTextLike := escapeLike(queryText)
	queryUsernameLike := escapeLike(queryUsername)
	...
	rows, err := s.db.QueryContext(ctx, `
		WITH bindings AS (
			...
			SELECT ? AS query_text, ? AS query_text_like,
			       ? AS query_username, ? AS query_username_like,
			       ? AS query_digits
		), candidates AS (
			       ...
			       CASE
			           ...
			           WHEN kind = 'username' THEN
			               CASE WHEN match_value = query_username THEN 0
			                    WHEN match_value LIKE query_username_like || '%' ESCAPE '\' THEN 1
			                    ELSE 2 END
			           ELSE CASE WHEN match_value = query_text THEN 0
			                     WHEN match_value LIKE query_text_like || '%' ESCAPE '\' THEN 1
			                     ELSE 2 END
			       END AS match_rank,
			...
			WHERE value IS NOT NULL AND TRIM(value) != '' AND (
				...
				OR (kind = 'username' AND query_username != ''
				 AND match_value LIKE '%' || query_username_like || '%' ESCAPE '\')
				OR (kind NOT IN ('phone', 'username')
				 AND match_value LIKE '%' || query_text_like || '%' ESCAPE '\')
			)
		), deduplicated AS (
		...
		LIMIT ?
	`, queryText, queryTextLike, queryUsername, queryUsernameLike, queryDigits, limit)
	if err != nil {

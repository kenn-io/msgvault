...
func (s *Store) buildLargeIndexesConcurrently(ctx context.Context) {
	...
	// before any fallible step below so it always runs, and on
	// context.Background() so a caller-cancelled ctx cannot skip it.
	defer func() {
...
func (s *Store) InitSchemaContext(ctx context.Context) error {
	...
	// transaction at all, so runMaintenance does not apply here).
	s.buildLargeIndexesConcurrently(context.Background())

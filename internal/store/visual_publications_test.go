...
func TestScopeEntrySeedSkipsNonCanonicalOwners(t *testing.T) {
	...
	f := storetest.New(t)
	generation := createVisualGeneration(t, f)
	messageID := f.CreateMessage("scope-seed-canonical")
	validHash := strings.Repeat("d4", 32)
	require.NoError(f.Store.UpsertAttachmentRecord(t.Context(), messageID, store.AttachmentWrite{
		Filename: "valid.png", MIMEType: "image/png", StoragePath: validHash[:2] + "/" + validHash,
		ContentHash: validHash, Size: 10, Role: store.AttachmentRoleStandalone,
		RoleSource: store.AttachmentRoleSourceImporterSemantics, SourcePartKey: "part:1",
	}))
	// A hashless URL-backed attachment: seeding it would create an owner
	// validateVisualOwner rejects, wedging every tombstone sweep.
	require.NoError(f.Store.UpsertAttachmentRecord(t.Context(), messageID, store.AttachmentWrite{
		Filename: "remote.png", MIMEType: "image/png",
		StoragePath: "https://cdn.example/attachments/remote.png",
		Size:        10, Role: store.AttachmentRoleStandalone,
		RoleSource: store.AttachmentRoleSourceImporterSemantics, SourcePartKey: "part:2",
	}))
	...
	rows, err := f.Store.DB().Query(f.Store.Rebind(
		`SELECT blob_hash FROM visual_publications WHERE generation_id = ? AND message_id = ?`),
		generation.ID, messageID)
	require.NoError(err)
	defer func() { _ = rows.Close() }()
	var seeded []string
	for rows.Next() {
		var hash string
		require.NoError(rows.Scan(&hash))
		seeded = append(seeded, hash)
	}
	require.NoError(rows.Err())
	assert.Equal([]string{validHash}, seeded,
		"only the canonical owner is seeded; the URL attachment is skipped")
}
...
func TestBodyDeletionBumpsContentStamp(t *testing.T) {
	...
	_, err := f.Store.DB().Exec(f.Store.Rebind(
		`INSERT INTO message_bodies (message_id, body_text) VALUES (?, ?)`),
		owner.MessageID, "text the embedding would capture")
	require.NoError(err)
	// Backdate the stamp: insert and delete within one millisecond would
	// otherwise produce identical values at the stamp's resolution.
	_, err = f.Store.DB().Exec(f.Store.Rebind(
		`UPDATE messages SET content_changed_at = ? WHERE id = ?`),
		time.Now().UTC().Add(-time.Hour), owner.MessageID)
	require.NoError(err)
	before, err := f.Store.GetVisualMessageContext(t.Context(), owner.MessageID)
	require.NoError(err)

	// Deletion is a content change: without a stamp bump, a body deleted
	// between the context snapshot and the claim passes the CAS and an
	// embedding of the deleted text publishes as current.
	_, err = f.Store.DB().Exec(f.Store.Rebind(
		`DELETE FROM message_bodies WHERE message_id = ?`), owner.MessageID)
	require.NoError(err)
	after, err := f.Store.GetVisualMessageContext(t.Context(), owner.MessageID)
	require.NoError(err)
	require.NotEqual(before.ContentStamp, after.ContentStamp,
		"deleting the body must advance content_changed_at")
}
...
func TestVisualPublicationRevisionTracksCurrentSet(t *testing.T) {
	...
	f := storetest.New(t)
	generation := createVisualGeneration(t, f)
	initial, err := f.Store.VisualPublicationRevision(t.Context(), generation.ID)
	require.NoError(err)
	again, err := f.Store.VisualPublicationRevision(t.Context(), generation.ID)
	require.NoError(err)
	assert.Equal(initial, again, "stable while nothing publishes")

	owner, attachmentID, sourceFence := createVisualOwner(t, f, "visual-revision")
	claim := claimVisualOwner(t, f, generation.ID, owner, "revision-1", sourceFence)
	token, err := f.Store.PrepareVisualPublication(t.Context(), store.PreparedVisualPublication{
		Claim: claim, RepresentativeAttachmentID: attachmentID,
		Role:       store.AttachmentRoleStandalone,
	...
	require.NoError(err)
	require.NoError(f.Store.CommitVisualPublication(t.Context(), claim, token))
	published, err := f.Store.VisualPublicationRevision(t.Context(), generation.ID)
	require.NoError(err)
	assert.NotEqual(initial, published, "a publish moves the revision")

	require.NoError(f.Store.TombstoneVisualOwner(t.Context(), generation.ID, owner, sourceFence+1))
	tombstoned, err := f.Store.VisualPublicationRevision(t.Context(), generation.ID)
	require.NoError(err)
	assert.NotEqual(published, tombstoned, "a tombstone moves the revision")
}

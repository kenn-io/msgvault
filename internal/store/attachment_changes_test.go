...
func TestAttachmentChangeConsumersPruneOnlySharedConsumedPrefix(t *testing.T) {
	...
	assert.Equal(1, attachmentChangeCount(t, f), "slower consumer retains its event")
	require.NoError(f.Store.AdvanceAttachmentChangeConsumer(
		t.Context(), "visual-index/v1", visualChanges[0].Sequence,
	))
	...
	createJournalAttachment(t, f, messageID, "part:2", "c")
...
func TestAttachmentChangeJournalCapturesCascadeDeletion(t *testing.T) {
	...
	assert.Equal("attachment_delete", changes[0].EventKind)

...
func TestPostgreSQLMergeSerializesWithObservationInsert(t *testing.T) {
	...
	absorbed, err := st.EnsureParticipantByIdentifier(
		"example", "merge-observation-absorbed", "Absorbed",
	)
	require.NoError(err)
	survivor, err := st.EnsureParticipantByIdentifier(
		"example", "merge-observation-survivor", "Survivor",
	)
	require.NoError(err)

	const advisoryKey int64 = 88442211
	...
	_, err = st.DB().ExecContext(ctx, fmt.Sprintf(`
		CREATE FUNCTION delay_merge_observation_insert_fn() RETURNS trigger LANGUAGE plpgsql AS $$
		BEGIN
			PERFORM pg_advisory_xact_lock(%d);
			RETURN NEW;
		END
		$$;
		CREATE TRIGGER delay_merge_observation_insert
		BEFORE INSERT ON participant_contact_observations
		FOR EACH ROW EXECUTE FUNCTION delay_merge_observation_insert_fn()`, advisoryKey))
	require.NoError(err)

	recordDone := make(chan error, 1)
	go func() {
		_, recordErr := st.RecordContactObservationContext(ctx, absorbed,
			store.ParticipantContactObservationInput{
				AddressKind: store.ContactAddressEmail, OriginalValue: "merge@example.com",
	...
	}, 5*time.Second, 10*time.Millisecond, "observation insert did not reach its barrier")

	mergeDone := make(chan error, 1)
	go func() { mergeDone <- st.MergeParticipants(absorbed, survivor) }()
	var mergeErr error
	mergeFinished := false
	require.Eventually(func() bool {
		select {
	...
	}, 5*time.Second, 10*time.Millisecond, "merge did not serialize behind observation recording")

	_, err = barrier.ExecContext(ctx, "SELECT pg_advisory_unlock($1)", advisoryKey)
	require.NoError(err)
	require.NoError(<-recordDone)
	if !mergeFinished {
		mergeErr = <-mergeDone
	}
	...
	assert.Equal("merge@example.com", observations[0].NormalizedValue)
}

func postgreSQLWaitingLockCount(t *testing.T, st *store.Store) int {
	t.Helper()
	var count int

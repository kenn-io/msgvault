...
var _ peoplebrowser.ProfileLister = (*PeopleBrowser)(nil)
...
func (b *PeopleBrowser) ListProfiles(ctx context.Context) ([]store.Person, error) {
	...
	profiles := make([]store.Person, len(resp.JSON200.People))
	for i := range resp.JSON200.People {
		profiles[i] = *personFromGenerated(&resp.JSON200.People[i])
	}
	return profiles, nil
}
...
func (b *PeopleBrowser) RelationshipCalendar(
	ctx context.Context, request peoplebrowser.CalendarRequest,
) (*query.RelationshipCalendarResponse, error) {
	if request.ParticipantID < 1 {
		return nil, errors.New("participant ID must be positive")
	}
	if err := validatePeopleRelationshipYear(request.Year, request.Timezone, time.Now()); err != nil {
		return nil, err
	}
		...
		func(client *apiclient.Client) (*generated.GetRelationshipCalendarResp, error) {
			return client.GetRelationshipCalendarWithResponse(ctx,
				&generated.GetRelationshipCalendarRequestOptions{
					PathParams: &generated.GetRelationshipCalendarPath{ID: request.ParticipantID},
					Body:       &body,
				})
		})
	if err != nil {
	...
	calendar, err := relationshipCalendarFromGenerated(*resp.JSON200)
	if err != nil {
		return nil, err
	}
	return &calendar, nil
...
func validatePeopleRelationshipYear(year int, timezone string, now time.Time) error {
	if timezone == "" {
		timezone = "UTC"
	}
	location, err := time.LoadLocation(timezone)
	if err != nil {
		return fmt.Errorf("relationship calendar timezone %q is invalid: %w", timezone, err)
	}
	if year < 1970 || year > now.In(location).Year() {
		return fmt.Errorf("relationship calendar year %d is outside the supported range", year)
	}
	return nil
}
...
func (b *PeopleBrowser) GetContact(
	...
	if err != nil {
		...
		if errors.As(err, &apiErr) && apiErr.Status == http.StatusNotFound &&
			...
			return nil, fmt.Errorf("%w: %w", peoplebrowser.ErrContactNotFound, err)
		}
		return nil, err
	}
...
// ListAttributesByUniversalID returns one portable definition group while
// retaining its archive-local slug for subsequent writes.
func (b *PeopleBrowser) ListAttributesByUniversalID(
	ctx context.Context, personID int64, universalID string,
) (*peoplebrowser.Attributes, error) {
	return b.listAttributes(ctx, personID, "", universalID)
}
			...
			return client.ListPersonAttributesWithResponse(ctx,
				&generated.ListPersonAttributesRequestOptions{
					...
					Query: &generated.ListPersonAttributesQuery{
						Slug: optionalString(slug), UniversalID: optionalString(universalID),
					},
				})
		})
	if err != nil {
...
func (b *PeopleBrowser) SetAttribute(
	...
	source := generated.SetPersonAttributeRequestSource(request.Source)
	if source == "" {
...
func (b *PeopleBrowser) AppendNote(
	...
	source := generated.AppendPersonNoteRequestSource(request.Source)
	if source == "" {
...
func personSummaryFromGenerated(person generated.PersonSummary) query.PersonSummary {
	...
	out := query.PersonSummary{
		...
		MeetingCount: person.MeetingCount, FileCount: person.FileCount,
		CurrentRelationshipTemperature: int(person.CurrentRelationshipTemperature),
		PeakRelationshipTemperature:    int(person.PeakRelationshipTemperature),
		PeakRelationshipYear:           int(person.PeakRelationshipYear),
		SourceCounts:                   counts,
...
func relationshipCalendarFromGenerated(
	calendar generated.RelationshipCalendarHTTPResponse,
) (query.RelationshipCalendarResponse, error) {
	days := make([]query.RelationshipCalendarDay, len(calendar.Days))
	for i, day := range calendar.Days {
		if day.ModalityMask < 0 || day.ModalityMask > 255 {
			return query.RelationshipCalendarResponse{}, fmt.Errorf(
				"decode relationship calendar: modality mask %d is outside uint8 range",
				day.ModalityMask,
			)
		}
		days[i] = query.RelationshipCalendarDay{
			Date: day.Date, Sent: day.Sent, Received: day.Received,
			Email: day.Email, Chat: day.Chat, Meetings: day.Meetings,
			Total: day.Total, ModalityMask: uint8(day.ModalityMask),
			Level: identityindex.HeatLevel(day.Level),
		}
	}
	annual := make([]identityindex.AnnualTemperatureSummary, len(calendar.Annual))
	for i, summary := range calendar.Annual {
		annual[i] = identityindex.AnnualTemperatureSummary{
			Year: int(summary.Year),
			TemperatureSummary: temperatureSummaryFromGenerated(
				generated.TemperatureSummary{
					Temperature: summary.Temperature, Rank: summary.Rank,
					Population: summary.Population, RawScore: summary.RawScore,
	...
	return query.RelationshipCalendarResponse{
		CanonicalID: calendar.CanonicalID, Year: int(calendar.Year),
		Timezone: calendar.Timezone, Days: days,
		Current: temperatureSummaryFromGenerated(calendar.Current), Annual: annual,
		PeakTemperature: int(calendar.PeakTemperature), PeakYear: int(calendar.PeakYear),
		ScoringTimezone: calendar.ScoringTimezone, ScoreVersion: int(calendar.ScoreVersion),
		EffectiveDate: calendar.EffectiveDate, CacheRevision: calendar.CacheRevision,
		IdentityRevision: calendar.IdentityRevision,
	}, nil
}

func temperatureSummaryFromGenerated(
	summary generated.TemperatureSummary,
) identityindex.TemperatureSummary {
	return identityindex.TemperatureSummary{
		Temperature: int(summary.Temperature), Rank: summary.Rank,
		Population: summary.Population, RawScore: summary.RawScore,
		Signals: identityindex.TemperatureSignals{
			SentSignal:     summary.Signals.SentSignal,
			ReceivedVolume: summary.Signals.ReceivedVolume,
			MeetingSignal:  summary.Signals.MeetingSignal,
			Modalities:     int(summary.Signals.Modalities),
		},
	}
}

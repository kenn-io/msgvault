...
func TestPeopleSearchDebounceLoadsBoundedTypedCompletions(t *testing.T) {
	...
	require.Len(t, backend.completionRequests, 1)
	assert.Equal(t, peoplebrowser.CompletionRequest{Query: "alice@example", Limit: peopleCompletionLimit}, backend.completionRequests[0])
	assert.Equal(t, uint64(9), loaded.requestID)
	assert.Equal(t, "alice@example", loaded.query)

	model = sendMsg(t, model, loaded)
	require.Len(t, model.peopleState.completions, 1)
	assert.Equal(t, "alice@example.test", model.peopleState.completions[0].Value)
	assert.Equal(t, "cache-7", model.peopleState.completionCacheRevision)
	assert.False(t, model.peopleState.completionLoading)
}
...
func TestPeopleCompletionRejectsStaleAndBlankQueriesDoNotLoad(t *testing.T) {
	...
	assert.Equal(t, "Old", model.peopleState.completions[0].DisplayLabel)

	updated, cmd := model.Update(peopleSearchDebounceMsg{
		query: "", debounceID: 5, requestID: 12, presentationGeneration: 8,
	})
	model = asModel(t, updated)
	assert.NotNil(t, cmd, "directory search still refreshes for a blank query")
	assert.Empty(t, backend.completionRequests)
	assert.Empty(t, model.peopleState.completions)
	assert.False(t, model.peopleState.completionLoading)
}
...
func TestPeopleSearchCompletionNavigationAcceptsAndOpensPerson(t *testing.T) {
	...
	model.peopleState.completions = []peoplebrowser.CompletionRow{
		...
		completionRow(9, "Alice Example", query.PeopleCompletionPhone, "+1 415 555 0100", "whatsapp"),
	}

	model, _ = sendKey(t, model, keyDown())
	assert.Equal(t, 1, model.peopleState.completionCursor)
	model, cmd := sendKey(t, model, tea.KeyPressMsg{Code: tea.KeyTab})
	assert.Equal(t, "+1 415 555 0100", model.peopleState.searchInput.Value())
	assert.NotNil(t, cmd)
	...
	assert.Equal(t, int64(9), model.peopleState.participantID)
	require.NotNil(t, open)
	loaded := runPeopleCommandMessage[peopleContactLoadedMsg](t, open)
	assert.Equal(t, int64(9), loaded.participantID)
}
...
func TestPeopleCompletionPanelRendersTypeSourceAndErrorsWithoutHidingDirectory(t *testing.T) {
	...
	assert.Contains(t, view, "Directory Person")
	assert.Contains(t, view, "Alice Example")
	assert.Contains(t, view, "organization")
	assert.Contains(t, view, "Example Labs")
	assert.Contains(t, view, "profile")
	assert.Contains(t, view, "completion unavailable")

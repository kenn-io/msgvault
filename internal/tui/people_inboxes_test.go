...
func TestPeopleInboxDaemonRevisionDriftRestartsOnceThenPauses(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "/api/v1/text/conversations", r.URL.Path)
		offset := 0
		...
		}`, revision.Load().(string), int64(offset+1), offset)
	...
	source := query.PersonInboxRow{SourceID: 21, SourceType: "beeper", SourceIdentifier: "whatsapp"}
	...
	assert.True(t, model.peopleState.conversationsRestarted)
	assert.Equal(t, uint64(6), model.peopleState.requestID)

	restarted := runPeopleCommandMessage[peopleConversationsLoadedMsg](t, restart)
	model = sendMsg(t, model, restarted)
	assert.Equal(t, "r2", model.peopleState.conversationsCacheRevision)
	...
	assert.ErrorIs(t, model.peopleState.inboxErr, errPeopleContentChanged)
}
...
func TestPeopleInboxNavigatesBeeperWhatsAppWithExactContactScope(t *testing.T) {
	...
	backend := &fakePeopleInboxBackend{
		...
		inboxes: &query.PersonInboxResponse{Rows: []query.PersonInboxRow{
			{SourceID: 21, SourceType: " Beeper ", SourceIdentifier: "whatsapp", LatestAt: latest},
			{SourceID: 22, SourceType: "beeper", SourceIdentifier: "signal", LatestAt: latest.Add(-time.Hour)},
			{SourceID: 23, SourceType: "BEEPER", SourceIdentifier: "whatsapp", LatestAt: latest.Add(-90 * time.Minute)},
		...
		detail: &query.MessageDetail{
			...
			Subject: "Test chat message", MessageType: "whatsapp",
	...
	assert.Equal(t, []int64{contact.ID}, backend.inboxRequests)
	assert.Equal(t, peopleLevelInboxTypes, model.peopleState.level)
	typeView := model.renderPeopleView()
	assert.Contains(t, typeView, "Beeper")
	assert.Contains(t, typeView, "Slack")
	assert.Equal(t, 1, strings.Count(typeView, "Beeper"))
	assert.NotContains(t, typeView, "WhatsApp")

	model, _ = sendKey(t, model, keyEnter())
	assert.Equal(t, peopleLevelInboxSources, model.peopleState.level)
	sourceView := model.renderPeopleView()
	assert.Contains(t, sourceView, "WhatsApp")
	assert.Contains(t, sourceView, "Signal")
	assert.Equal(t, 2, strings.Count(sourceView, "WhatsApp"),
		"concrete source rows must not collapse when identifiers match")
	assert.NotContains(t, sourceView, "test-workspace")
	...
	require.Len(t, backend.conversationRequests, 1)
	filter := backend.conversationRequests[0].filter
	require.NotNil(t, filter.SourceID)
	assert.Equal(t, int64(21), *filter.SourceID)
	assert.Equal(t, []int64{7, 9, 11}, filter.ParticipantIDs)
	assert.Equal(t, query.Pagination{Limit: peoplePageSize}, filter.Pagination)
	assert.Equal(t, query.TextSortByLastMessage, filter.SortField)
	assert.Equal(t, query.SortDesc, filter.SortDirection)
	assert.Equal(t, peopleLevelConversations, model.peopleState.level)
	assert.Contains(t, model.renderPeopleView(), "Test chat")

	model, cmd = sendKey(t, model, keyEnter())
	loadedMessages := runPeopleCommandMessage[peopleConversationMessagesLoadedMsg](t, cmd)
	model = sendMsg(t, model, loadedMessages)

	require.Len(t, backend.messageRequests, 1)
	assert.Equal(t, int64(101), backend.messageRequests[0].conversationID)
	assert.Equal(t, int64(21), *backend.messageRequests[0].filter.SourceID)
	assert.Equal(t, []int64{7, 9, 11}, backend.messageRequests[0].filter.ParticipantIDs)
	assert.Equal(t, peopleLevelConversation, model.peopleState.level)
	assert.Contains(t, model.renderPeopleView(), "Exact participant-scoped hello")

	model, cmd = sendKey(t, model, keyEnter())
	loadedDetail := runPeopleCommandMessage[peopleMessageLoadedMsg](t, cmd)
	model = sendMsg(t, model, loadedDetail)

	assert.Equal(t, []int64{501}, backend.detailRequests)
	assert.Equal(t, peopleLevelMessage, model.peopleState.level)
	detailView := model.renderPeopleView()
	assert.Contains(t, detailView, "Existing detail renderer body")
	assert.Contains(t, detailView, "test-note.txt")

	model, _ = sendKey(t, model, keyEsc())
	assert.Equal(t, peopleLevelConversation, model.peopleState.level)
	assert.Equal(t, 0, model.textState.cursor)
}
...
func TestPeopleInboxSourceRowsShowDirectionalCountsAndLatestReceived(t *testing.T) {
	...
	assert.Contains(t, view, "latest received 2026-08-20 16:30")
	assert.Contains(t, view, "sent 7")
	assert.Contains(t, view, "received 12")
	assert.Contains(t, view, "conversations 3")

	model.width = 52
	assert.Contains(t, model.renderPeopleView(), "WhatsApp")
}
...
func TestPeopleInboxConversationRetryKeepsContactAndSource(t *testing.T) {
	...
	require.Error(t, model.peopleState.inboxErr)
	assert.Equal(t, peopleLevelConversations, model.peopleState.level)

	model, cmd := sendKey(t, model, key('r'))

	require.NotNil(t, cmd)
	assert.Equal(t, contact.ID, model.peopleState.participantID)
	require.NotNil(t, model.peopleState.selectedInboxSource)
	assert.Equal(t, source.SourceID, model.peopleState.selectedInboxSource.SourceID)
	loaded := runPeopleCommandMessage[peopleConversationsLoadedMsg](t, cmd)
	model = sendMsg(t, model, loaded)
	assert.NoError(t, model.peopleState.inboxErr)
	assert.Equal(t, peopleLevelConversations, model.peopleState.level)
	assert.Contains(t, model.renderPeopleView(), "Recovered chat")
}
...
func TestPeopleInboxPaginatesAllSourceConversationsWithoutLosingSelection(t *testing.T) {
	...
	require.Len(t, model.peopleState.conversations, peoplePageSize)

	model, cmd = sendKey(t, model, key('G'))
	require.NotNil(t, cmd, "reaching the end should request the next source page")
	assert.Equal(t, peoplePageSize-1, model.peopleState.cursor)
	model = sendMsg(t, model, runPeopleCommandMessage[peopleConversationsLoadedMsg](t, cmd))
	require.Len(t, model.peopleState.conversations, 2*peoplePageSize)
	assert.Equal(t, peoplePageSize-1, model.peopleState.cursor)

	model, cmd = sendKey(t, model, key('G'))
	require.NotNil(t, cmd)
	model = sendMsg(t, model, runPeopleCommandMessage[peopleConversationsLoadedMsg](t, cmd))
	assert.Len(t, model.peopleState.conversations, 205)
	assert.True(t, model.peopleState.conversationsComplete)
	require.Len(t, backend.conversationRequests, 3)
	assert.Equal(t, []int{0, 100, 200}, []int{
...
func TestPeopleInboxPaginatesAllConversationMessagesAndDeduplicatesBoundary(t *testing.T) {
	...
	require.Len(t, model.textState.messages, peoplePageSize)

	model, cmd = sendKey(t, model, key('G'))
	require.NotNil(t, cmd, "reaching the end should request the next message page")
	model = sendMsg(t, model, runPeopleCommandMessage[peopleConversationMessagesLoadedMsg](t, cmd))
	assert.Len(t, model.textState.messages, 199, "message 100 appears in both pages")
	assert.True(t, model.peopleState.conversationComplete)
	require.Len(t, backend.messageRequests, 2)
	assert.Equal(t, peoplePageSize, backend.messageRequests[1].filter.Pagination.Offset)
}
...
func TestPeopleInboxDrillSettlesAncestorAppend(t *testing.T) {
	...
	t.Run("conversation append while opening conversation", func(t *testing.T) {
		...
		require.NotNil(t, pendingAppend)
		require.True(t, model.peopleState.conversationsLoading)

		model, childLoad := sendKey(t, model, keyEnter())
		require.NotNil(t, childLoad)
		assert.Equal(t, peopleLevelConversation, model.peopleState.level)
		assert.Equal(t, conversations, model.peopleState.conversations)
		assert.False(t, model.peopleState.conversationsLoading)
		assert.True(t, model.peopleState.conversationLoading)
		...
		assert.Equal(t, peopleLevelConversations, model.peopleState.level)
		assert.Equal(t, conversations, model.peopleState.conversations)
		assert.Equal(t, 100, model.peopleState.conversationsNextOffset)
		assert.Equal(t, "r1", model.peopleState.conversationsCacheRevision)
		resumed := model.maybeLoadMorePeopleInbox()
		assert.NotNil(t, resumed)
		assert.True(t, model.peopleState.conversationsLoading)
	})
	...
	t.Run("message append while opening detail", func(t *testing.T) {
		...
		pendingAppend := model.maybeLoadMorePeopleInbox()
		require.NotNil(t, pendingAppend)
		require.True(t, model.peopleState.conversationLoading)

		model, detailLoad := sendKey(t, model, keyEnter())
		require.NotNil(t, detailLoad)
		assert.Equal(t, peopleLevelMessage, model.peopleState.level)
		assert.Equal(t, messages, model.textState.messages)
		assert.False(t, model.peopleState.conversationLoading)
		...
		assert.Equal(t, peopleLevelConversation, model.peopleState.level)
		assert.Equal(t, messages, model.textState.messages)
		assert.Equal(t, 100, model.peopleState.conversationNextOffset)
		assert.Equal(t, "r1", model.peopleState.conversationCacheRevision)
		resumed := model.maybeLoadMorePeopleInbox()
		assert.NotNil(t, resumed)
		assert.True(t, model.peopleState.conversationLoading)
	})
...
func TestPeopleInboxPageRevisionDriftRestartsOnceThenPauses(t *testing.T) {
	...
	t.Run("source conversations", func(t *testing.T) {
		...
		assert.ErrorIs(t, model.peopleState.inboxErr, errPeopleContentChanged)
		assert.Equal(t, int64(3), model.peopleState.conversations[0].ConversationID)
	})
	...
	t.Run("conversation messages", func(t *testing.T) {
		...
		model = asModel(t, updated)
		require.NotNil(t, restart)
		assert.True(t, model.peopleState.conversationRestarted)
		...
		assert.ErrorIs(t, model.peopleState.inboxErr, errPeopleContentChanged)
		assert.Equal(t, int64(3), model.textState.messages[0].ID)
	})
}
...
func TestPeopleInboxLeavingDuringLoadReturnsToRetryableSameSource(t *testing.T) {
	...
	model = sendMsg(t, model, loaded)
	assert.NoError(t, model.peopleState.inboxErr)
	assert.Contains(t, model.renderPeopleView(), "Resumed chat")
}
...
func TestPeopleInboxBackRestoresEveryListPosition(t *testing.T) {
	...
	model = sendMsg(t, model, runPeopleCommandMessage[peopleConversationMessagesLoadedMsg](t, cmd))
	model, _ = sendKey(t, model, key('j'))
	assert.Equal(t, 1, model.textState.cursor)
	...
	assert.Equal(t, peopleLevelInboxSources, model.peopleState.level)
	assert.Equal(t, 1, model.peopleState.cursor)
	assert.Equal(t, 1, model.peopleState.scrollOffset)
}
...
func TestPeopleInboxBackRestoresVisibleSourceAtConstrainedHeight(t *testing.T) {
	...
	assert.Equal(t, 5, model.peopleState.cursor)
	assert.Equal(t, 4, model.peopleState.scrollOffset)
	assert.Contains(t, stripANSI(model.renderPeopleView()), "▶ Network 5")
}
...
func TestPeopleInboxTabLeaveSettlesPendingLoadAndReentryStartsFresh(t *testing.T) {
	...
	require.True(t, model.peopleState.inboxesLoading)

	model, _ = sendKey(t, model, keyTab())

	assert.Equal(t, peopleTabMeetings, model.peopleState.tab)
	assert.Equal(t, peopleLevelContact, model.peopleState.level)
	assert.False(t, model.peopleState.inboxesLoading)
	assert.False(t, model.peopleState.conversationsLoading)
	assert.False(t, model.peopleState.conversationLoading)
	assert.False(t, model.peopleState.messageLoading)
	assert.False(t, model.loading)

	stale := runPeopleCommandMessage[peopleInboxesLoadedMsg](t, pending)
	model = sendMsg(t, model, stale)
	assert.False(t, model.peopleState.inboxesLoaded)
	assert.Empty(t, model.peopleState.inboxTypes)

	model, fresh := sendKey(t, model, keyShiftTab())

	require.NotNil(t, fresh)
	assert.Equal(t, peopleTabInboxes, model.peopleState.tab)
	assert.True(t, model.peopleState.inboxesLoading)
	loaded := runPeopleCommandMessage[peopleInboxesLoadedMsg](t, fresh)
	model = sendMsg(t, model, loaded)
	require.Len(t, backend.inboxRequests, 2)
	assert.Equal(t, []int64{contact.ID, contact.ID}, backend.inboxRequests)
	assert.True(t, model.peopleState.inboxesLoaded)
	require.Len(t, model.peopleState.inboxTypes, 1)
	assert.Equal(t, "beeper", model.peopleState.inboxTypes[0].key)
}
...
func TestPeopleInboxTabLeaveSettlesEverySubloadWithoutDiscardingCache(t *testing.T) {
	...
	assert.Equal(t, "beeper", model.peopleState.inboxTypes[0].key)
	require.Len(t, model.peopleState.conversations, 1)
	assert.Equal(t, "Cached chat", model.peopleState.conversations[0].Title)
}

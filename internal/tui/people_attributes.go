...
func (m Model) handlePeopleNewFieldKey(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	if form.fieldFocus == peopleFieldFocusName && !form.submitting {
		switch msg.String() {
		case keyNameEsc, "ctrl+c", "tab", "shift+tab", keyNameEnter:
		default:
			var cmd tea.Cmd
...
func (m Model) submitPeopleAttribute() (tea.Model, tea.Cmd) {
	...
	if form.editing || form.definition.Cardinality == store.AttributeCardinalityMulti {
...
func (m Model) peopleNotesAttribute() (
	...
	for _, group := range m.peopleState.attributes.Groups {
		if group.Definition.Slug != store.AttributeSlugNotes {
			continue
		}
...
func (m Model) openPeopleAttributeAdd(group peoplebrowser.AttributeGroup) (tea.Model, tea.Cmd) {
	...
	if group.Definition.Cardinality == store.AttributeCardinalityMulti {
		ordinal = firstUnusedPeopleOrdinal(group.Current)
	}
	m.peopleState.attributesNotice = ""
	m.peopleState.form = newPeopleValueForm(group.Definition, nil, ordinal)
	return m, nil
}
...
func firstUnusedPeopleOrdinal(values []store.PersonAttributeValue) int64 {
	used := make(map[int64]struct{}, len(values))
	for _, value := range values {
		if value.Ordinal >= 0 {
			used[value.Ordinal] = struct{}{}
		}
	}
...
func (m Model) peopleNewFieldFormView() string {
	...
	lines := []string{
		...
		line(peopleFieldFocusSave, "Save", "Enter"),
	}
	if form.submitting {

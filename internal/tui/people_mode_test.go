...
func TestPeopleContactNotFoundReturnsToRefreshedDirectoryAndRejectsStaleResults(t *testing.T) {
	...
	assert.Nil(t, model.peopleState.contact)
	assert.Zero(t, model.peopleState.participantID)
	assert.Empty(t, model.peopleState.breadcrumbs)
	assert.Equal(t, "merged", model.peopleState.searchQuery)
	require.NotNil(t, directory)
	assert.Contains(t, model.peopleState.directoryNotice, "no longer available")
	...
	assert.Empty(t, model.peopleState.rows, "the old request cannot repopulate the recovered directory")

	model = sendMsg(t, model, peopleLoadedMessage(t, directory))
	require.Len(t, model.peopleState.rows, 1)
	assert.Equal(t, replacement.ID, model.peopleState.rows[0].ID)
	assert.Contains(t, stripANSI(model.renderPeopleView()), "no longer available")
	assert.Equal(t, []int64{contact.ID}, backend.contactRequests, "the missing contact is not reloaded in a loop")
}
...
func TestPeopleContactRetryKeepsCurrentWorkspace(t *testing.T) {
	...
	require.Len(t, model.peopleState.breadcrumbs, 1)
	...
	assert.Equal(t, int64(2), loaded.participantID)
	assert.Equal(t, []int64{2}, backend.contactRequests)
}
...
func testPerson(id int64, label string) query.PersonSummary {
	return query.PersonSummary{
		...
		Identifiers: []query.PersonIdentifier{{
			Type: "email", Value: fmt.Sprintf("person-%d@example.test", id),
...
func TestNextModeIncludesPeople(t *testing.T) {
	assert.Equal(t, modeTexts, nextMode(modeEmail, true, true))
	assert.Equal(t, modeMeetings, nextMode(modeTexts, true, true))
	assert.Equal(t, modePeople, nextMode(modeMeetings, true, true))
	assert.Equal(t, modeEmail, nextMode(modePeople, true, true))
	assert.Equal(t, modeMeetings, nextMode(modeEmail, false, true))
	assert.Equal(t, modePeople, nextMode(modeMeetings, false, true))
...
func TestEnteringPeopleLoadsFirstDirectoryPage(t *testing.T) {
	...
	require.True(t, handled)
	assert.Equal(t, modePeople, updated.mode)
	loaded := peopleLoadedMessage(t, cmd)
	require.Len(t, backend.searchRequests, 1)
	assert.Equal(t, peoplebrowser.SearchRequest{Limit: peoplePageSize}, backend.searchRequests[0])
	assert.Equal(t, updated.presentationGeneration, loaded.presentationGeneration)
	assert.Equal(t, updated.peopleState.requestID, loaded.requestID)

	updated = sendMsg(t, updated, loaded)
	require.Len(t, updated.peopleState.rows, 1)
	assert.Equal(t, "Test Person", updated.peopleState.rows[0].DisplayLabel)
	assert.True(t, updated.peopleState.initialized)
...
func TestPeopleSearchDebounceRejectsStaleResponses(t *testing.T) {
	...
	assert.True(t, model.peopleState.searchActive)
	assert.True(t, model.peopleState.searchInput.Focused())

	model, _ = sendKey(t, model, key('m'))
	requestID := model.peopleState.requestID
	debounceID := model.peopleState.searchDebounceID
	assert.Greater(t, requestID, uint64(4))
	...
	require.Len(t, model.peopleState.rows, 1)
	assert.Equal(t, "Original Person", model.peopleState.rows[0].DisplayLabel)
	...
	loaded := peopleLoadedMessage(t, cmd)
	assert.Equal(t, "m", backend.searchRequests[0].Query)
	assert.Equal(t, requestID, loaded.requestID)

	model = sendMsg(t, model, loaded)
	require.Len(t, model.peopleState.rows, 1)
	assert.Equal(t, "Matching Person", model.peopleState.rows[0].DisplayLabel)
	assert.Equal(t, "m", model.peopleState.searchQuery)
}
...
func TestPeopleSearchEscapeRestartsCommittedQueryAfterDebounceLoad(t *testing.T) {
	...
	model = asModel(t, updated)
	require.NotNil(t, previewLoad)
	assert.Empty(t, model.peopleState.rows)

	model, replacement := sendKey(t, model, keyEsc())
	assert.False(t, model.peopleState.searchActive)
	assert.Equal(t, "committed", model.peopleState.searchQuery)
	assert.Equal(t, "committed", model.peopleState.searchInput.Value())
	loaded := peopleLoadedMessage(t, replacement)
	require.Len(t, backend.searchRequests, 1)
	assert.Equal(t, "committed", backend.searchRequests[0].Query)

	model = sendMsg(t, model, loaded)
	require.Len(t, model.peopleState.rows, 1)
	assert.Equal(t, "Restored Result", model.peopleState.rows[0].DisplayLabel)
	assert.True(t, model.peopleState.initialized)
}
...
func TestPeopleSearchEscapeRestartsFreshLoadInvalidatedBeforeDebounce(t *testing.T) {
	...
	require.NotNil(t, initialFreshLoad)
	assert.False(t, model.peopleState.initialized)
	assert.Empty(t, model.peopleState.rows)
	model, _ = sendKey(t, model, key('/'))
	model, _ = sendKey(t, model, key('x'))
	assert.False(t, model.peopleState.directoryLoading)
	assert.Equal(t, "committed", model.peopleState.searchQuery)
...
func TestPeopleSearchFocusCancelPreservesPaginatedDirectory(t *testing.T) {
	...
	assert.True(t, model.peopleState.loadingMore)
	model, _ = sendKey(t, model, key('/'))
	model, cancel := sendKey(t, model, keyEsc())

	assert.Nil(t, cancel)
	require.Len(t, model.peopleState.rows, 10)
	assert.Equal(t, "Person 1", model.peopleState.rows[0].DisplayLabel)
	assert.Equal(t, 5, model.peopleState.cursor)
	assert.Equal(t, 2, model.peopleState.scrollOffset)
	assert.Equal(t, "next-page", model.peopleState.nextCursor)
	assert.True(t, model.peopleState.initialized)
	assert.False(t, model.peopleState.directoryLoading)
	assert.False(t, model.peopleState.loadingMore)

	model, _ = sendKey(t, model, tea.KeyPressMsg{Code: tea.KeyUp})
	model, resumed := sendKey(t, model, keyDown())
...
func TestPeopleContactBackRestoresDirectoryPosition(t *testing.T) {
	...
	assert.Equal(t, peopleLevelContact, model.peopleState.level)
	assert.Equal(t, int64(2), model.peopleState.participantID)
	require.Len(t, model.peopleState.breadcrumbs, 1)

	loaded := cmd()
	contactLoaded, ok := loaded.(peopleContactLoadedMsg)
	require.True(t, ok)
	assert.Equal(t, model.peopleState.requestID, contactLoaded.requestID)
	assert.Equal(t, model.presentationGeneration, contactLoaded.presentationGeneration)
	assert.Equal(t, int64(2), contactLoaded.participantID)
	model = sendMsg(t, model, contactLoaded)
	require.NotNil(t, model.peopleState.contact)
	assert.Equal(t, "Second Person", model.peopleState.contact.DisplayLabel)
...
func TestReenteringPeopleReloadsAbandonedContactRequest(t *testing.T) {
	...
	assert.Equal(t, uint64(6), loaded.requestID)
	assert.Equal(t, updated.presentationGeneration, loaded.presentationGeneration)
	assert.Equal(t, int64(2), loaded.participantID)
}
...
func TestPeopleDirectoryRefreshReentryRestartsAbandonedLoad(t *testing.T) {
	...
	require.Len(t, backend.searchRequests, 1)
	assert.Empty(t, backend.searchRequests[0].Cursor)
	model = sendMsg(t, model, loaded)
	require.Len(t, model.peopleState.rows, 1)
	assert.Equal(t, "Refreshed Person", model.peopleState.rows[0].DisplayLabel)
}
...
func TestPeopleSearchDebounceLoadReentryRestartsAbandonedLoad(t *testing.T) {
	...
	loaded := peopleLoadedMessage(t, reloaded)
	require.Len(t, backend.searchRequests, 1)
	assert.Empty(t, backend.searchRequests[0].Query)
	model = sendMsg(t, model, loaded)
	require.Len(t, model.peopleState.rows, 1)
	assert.Equal(t, "Search Result", model.peopleState.rows[0].DisplayLabel)
}
...
func TestPeoplePaginationLoadsWithinLastFiveRows(t *testing.T) {
	...
	require.True(t, ok)
	assert.True(t, page.append)
	require.Len(t, backend.searchRequests, 1)
	assert.Equal(t, "next-page", backend.searchRequests[0].Cursor)

	model = sendMsg(t, model, page)
	require.Len(t, model.peopleState.rows, 11)
	assert.Equal(t, "Next Page", model.peopleState.rows[10].DisplayLabel)
}
...
func TestPeoplePaginationRestartsOnceAfterCacheDrift(t *testing.T) {
	...
	model = asModel(t, updated)
	assert.Empty(t, model.peopleState.rows)
	assert.Zero(t, model.peopleState.cursor)
	assert.Zero(t, model.peopleState.scrollOffset)
	assert.True(t, model.peopleState.paginationRestarted)
	assert.NoError(t, model.peopleState.err)
	...
	model = sendMsg(t, model, loaded)
	require.Len(t, model.peopleState.rows, 1)
	assert.Equal(t, "Restarted Person", model.peopleState.rows[0].DisplayLabel)
	...
	model = asModel(t, updated)
	assert.Nil(t, cmd)
	require.Error(t, model.peopleState.err)
	assert.True(t, errors.Is(model.peopleState.err, errPeopleDirectoryChanged))
	assert.Contains(t, model.peopleState.err.Error(), "retry")
}
...
func TestPeopleSecondPaginationDriftPausesUntilExplicitRetry(t *testing.T) {
	...
	assert.Contains(t, view, "Partial Person")
	...
	model = sendMsg(t, model, loaded)
	require.Len(t, model.peopleState.rows, 1)
	assert.Equal(t, "Reloaded Person", model.peopleState.rows[0].DisplayLabel)
	assert.NoError(t, model.peopleState.err)
}

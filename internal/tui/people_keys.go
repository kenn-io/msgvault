...
func (m Model) activatePeopleTab(tab peopleTab) (Model, tea.Cmd) {
	...
	return m, tea.Batch(
		spinCmd,
		m.loadPeopleAttributes(m.peopleState.contact.Profile.ID, tab),
	)
}
...
func (m Model) handlePeopleOverviewKey(
	...
	case "n":
		...
		if value == nil {
			updated, cmd := m.openPeopleAttributeAdd(group)
			return updated, cmd, true
		}
		updated, cmd := m.openPeopleAttributeEdit(group, value)
		return updated, cmd, true
	case "r":
		if m.peopleState.err != nil {
			return m, nil, false
...
func (m Model) handlePeopleActivityMessageKey(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	if msg.String() == keyNameEsc || msg.String() == "backspace" {
...
func (m Model) handlePeopleSearchInput(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	case keyNameEnter:
		...
		m.peopleState.clearCompletions()
		return m.startPeopleDirectoryLoad(false)
	case keyNameEsc:
...
func (m Model) handlePeopleSearchDebounce(msg peopleSearchDebounceMsg) (tea.Model, tea.Cmd) {
	...
	updated, directoryCmd := m.startPeopleDirectoryLoad(false)
	m = updated.(Model)
	if msg.query == "" {
		m.peopleState.clearCompletions()
...
func (m Model) startPeopleDirectoryLoad(appendResults bool) (tea.Model, tea.Cmd) {
	cursor := ""
	if appendResults {
		cursor = m.peopleState.nextCursor
	} else {
	...
	m.peopleState.loadingMore = appendResults
	m.loading = true
	spinCmd := m.startSpinner()
	return m, tea.Batch(spinCmd, m.loadPeopleDirectory(cursor, appendResults))
}

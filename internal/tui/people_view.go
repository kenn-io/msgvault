...
func (m Model) peopleHeaderView() string {
	...
	if m.peopleState.level == peopleLevelDirectory {
		line1 := m.styles.titleBar.Render(padRight(
			titleText+" [People]", max(m.width-2, 0),
		))
...
func peopleDirectoryHeader(columns peopleDirectoryColumns) string {
	row := listIndicatorBlank + peopleCell("Label", columns.labelWidth)
...
func peopleDirectoryRow(person query.PersonSummary, selected bool, columns peopleDirectoryColumns) string {
	...
	if columns.showCounts {
		row += "  " + peopleCell(strconv.FormatInt(person.ActivityCount, 10), 8)
		row += "  " + peopleCell(strconv.FormatInt(person.FileCount, 10), 5)
	}
	return row
}
...
func (m Model) peopleDirectoryView() string {
	...
	if len(m.peopleState.rows) == 0 {
		...
		if message != "" {
			sb.WriteString(style.Render(padRight(listIndicatorBlank+message, max(m.width, 0))))
			sb.WriteString("\n")
			used = 1
		}
	}
...
func (m Model) peopleCompletionPanelLines(limit int) []peopleCompletionPanelLine {
	...
	if len(lines) < limit && m.peopleState.completionErr != nil {
		lines = append(lines, peopleCompletionPanelLine{
			text: listIndicatorBlank + textutil.SanitizeTerminal(m.peopleState.completionErr.Error()), isError: true,
		})
...
func (m Model) peopleContactView() string {
	...
	visible := m.visibleRows()
	start := 0
	if m.peopleState.tab == peopleTabOverview && m.peopleState.level == peopleLevelContact {
		start = min(m.peopleState.scrollOffset, max(len(lines)-visible, 0))
	}
	for i := range visible {
		line := ""
...
func (m Model) peopleContactStatus() (string, bool) {
	...
	case peopleTabOverview:
		...
		if m.peopleState.relationshipLoading {
			return "Loading relationship activity...", true
		}
...
func (m Model) peopleOverviewRelationshipLines() []string {
	year := m.peopleState.relationshipYear
	heading := relationshipAlignedLine("● Relationship", strconv.Itoa(year), max(m.width, 1))
	switch {
	case m.peopleState.relationshipLoading && m.peopleState.relationshipCalendar == nil:
		return []string{"", heading, m.spinnerIndicator() + " Loading relationship activity..."}
	case m.peopleState.relationshipErr != nil:
		return []string{"", heading, m.peopleState.relationshipErr.Error() + " [r retry]"}
	case m.peopleState.relationshipCalendar == nil:
		return []string{"", heading, "Relationship activity has not loaded. Press r to retry."}
	default:
		return append([]string{""}, relationshipHeatmapLines(
			m.peopleState.relationshipCalendar,
			max(m.width, 1),
			m.styles,
			noColorRequested(),
		)...)
...
func peopleActivitySource(entry query.EntryRow) string {
	...
	if entry.MessageType == emailMessageType || strings.Contains(strings.ToLower(entry.SourceType), "mail") {
		return "Email"
	}
...
func (m Model) peopleInboxLines() []string {
	...
	case peopleLevelDirectory, peopleLevelContact, peopleLevelMeetingDetail,
...
func (m Model) peopleFooterView() string {
	...
	if m.peopleState.level != peopleLevelDirectory {
		...
		if m.peopleState.level == peopleLevelContact &&
			...
			keys = []string{
				helpLabelVertical, "PgUp/PgDn", helpLabelEnter,
			}
	...
	if m.width < 60 {
		if m.peopleState.level == peopleLevelDirectory {
			...
			if m.peopleState.searchActive {
				keys = []string{helpLabelVertical, "Tab", helpLabelEnter, helpLabelEsc, "?"}
			}
		} else {
			...
			if m.peopleState.level == peopleLevelContact &&
				(m.peopleState.tab == peopleTabMeetings || m.peopleState.tab == peopleTabFiles ||
					m.peopleState.tab == peopleTabActivity) {
				keys = []string{helpLabelVertical, "Pg", helpLabelEnter}
				if m.peopleContentTabRetryAvailable() {
					keys = append(keys, "r")
				}
				keys = append(keys, "Tab", helpLabelEsc)
			}
			if m.peopleState.level >= peopleLevelInboxTypes {
				keys = []string{helpLabelVertical, helpLabelEnter, helpLabelEsc, "r", "m", "?"}
			}
			if m.peopleState.level == peopleLevelMessage {
				keys = []string{helpLabelVertical, "/", helpLabelEsc, "r", "m", "?"}
			}
			if m.peopleState.level == peopleLevelMeetingDetail {
				keys = []string{helpLabelVertical, "/", "r", helpLabelEsc, "m", "?"}
			}
			if m.peopleState.level == peopleLevelActivityMessage {
				keys = []string{helpLabelVertical, "/", "r", helpLabelEsc, "m", "?"}
			}

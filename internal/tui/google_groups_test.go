...
func TestGoogleGroupsLabelsSanitizedAtDisplay(t *testing.T) {
	...
	require.Equal(t, int64(1), summary.MessagesAdded)
	var id int64
	require.NoError(t, st.DB().QueryRow("SELECT id FROM messages").Scan(&id))
	engine := query.NewSQLiteEngine(st.DB())
	detail, err := engine.GetMessage(t.Context(), id)
	require.NoError(t, err)
	require.Contains(t, detail.Labels, label, "archive reads retain the original label")
	...
	assert.NotContains(t, output, "\x1b]52;")
	assert.NotContains(t, output, "\x1b]8;")
	assert.Contains(t, output, "Résolulink")
	assert.Contains(t, detail.Labels, label, "rendering must not mutate the original label")
}

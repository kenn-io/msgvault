...
func TestNewFieldDefinitionInputMapsEditableKinds(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "Favorite "+tt.name, got.Label)
			assert.Equal(t, tt.valueType, got.ValueType)
			assert.Equal(t, tt.fieldType, got.FieldType)
			assert.Equal(t, store.AttributeCardinalityMulti, got.Cardinality)
		})
	}
}

...
func beginConfigReplacement(left, right string, expectedLeft, expectedRight ConfigFile) (configReplacement, error) {
	...
	if err := unix.RenameatxNp(dir, leftName, dir, rightName, unix.RENAME_SWAP); err != nil {
		_ = displaced.Close()
		_ = unix.Close(dir)
		return configReplacement{}, fmt.Errorf("rename swap: %w", err)
	}
	return configReplacement{
		...
		rollbackPublished: func(expected ConfigFile) error {
			...
			if err := unix.RenameatxNp(dir, leftName, dir, rightName, unix.RENAME_SWAP); err != nil {
				return fmt.Errorf("rollback rename swap: %w", err)
			}
			return nil
		},

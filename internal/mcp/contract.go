...
func (c *confirmationChallenges) issue(session *sdkmcp.ServerSession, toolName string, arguments map[string]any, message string) (string, error) {
	...
	c.pending[token] = confirmationChallenge{session: session, digest: digest, expires: now.Add(confirmationChallengeTTL)}
	return token, nil
}

func (c *confirmationChallenges) consume(token string, session *sdkmcp.ServerSession, toolName string, arguments map[string]any, message string) bool {
	...
	return challenge.session == session && challenge.expires.After(c.now()) && challenge.digest == digest
}
...
func (r toolRequest) confirmUserAction(ctx context.Context, message string) error {
	...
	if response, ok := r.inputResponses["confirm"]; ok {
		...
		if r.confirmations == nil || !r.confirmations.consume(r.requestState, r.session, r.toolName, r.arguments, message) {

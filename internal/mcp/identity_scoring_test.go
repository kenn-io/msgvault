...
func TestIdentityScoringMCPAcceptsZeroHistoryFilters(t *testing.T) {
	assert := assert.New(t)
	var query string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal("/api/v1/identity/scoring/history", r.URL.Path)
		query = r.URL.RawQuery
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{"judgments":[],"limit":20,"candidate_id":0}`)
	}))
	t.Cleanup(server.Close)
	client, err := daemonclient.New(daemonclient.Config{URL: server.URL, AllowInsecure: true, HTTPClient: server.Client()})
	require.NoError(t, err)

	result := rawCallTool(t, ServeOptions{Engine: &querytest.MockEngine{}, IdentityScoring: client},
		ToolListIdentityJudgments,
		map[string]any{"candidate_id": float64(0), "before_id": float64(0)})
	assert.NotEqual(true, result["isError"])
	assert.Equal("limit=20", query, "zero filters should be omitted while preserving the MCP default limit")
}

...
func (s *Server) handleReviewedIdentityMatchDecision(
	...
	candidate, revision, err := matches.DecideIdentityMatchReviewedContext(
	...
	if current, readErr := matches.GetIdentityMatchReviewContext(r.Context(), id); readErr == nil {
		candidate = current
	}
	if decision == store.IdentityMatchStateAccepted {
		writeJSON(w, http.StatusOK, IdentityMatchAcceptResponse{
			Candidate: *candidate, IdentityRevision: revision,

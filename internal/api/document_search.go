...
type DocumentVectorStatusStore interface {
	...
	GetDocumentVectorOperationsStatus(ctx context.Context, configured store.DocumentVectorGenerationSpec, egressFingerprint string, generationID int64, afterToken string, limit int) (store.DocumentVectorOperationsStatus, error)
}
...
func (s *Server) handleDocumentVectorStatus(w http.ResponseWriter, r *http.Request) {
	...
	spec := store.DocumentVectorGenerationSpec{
		Fingerprint: vectordocument.Fingerprint(target, s.cfg.Vector), TargetExtractionProfileID: target,
	...
	egressFingerprint, err := vectordocument.EgressFingerprint(target, s.cfg.Vector)
	if err != nil {
		s.writeDocumentSearchError(w, err)
		return
	}
	status, err := statusStore.GetDocumentVectorOperationsStatus(r.Context(), spec, egressFingerprint, generationID, r.URL.Query().Get("after_token"), limit)
	if err != nil {
		s.writeDocumentSearchError(w, err)
		return
	}
	s.documentSearchMu.RLock()
	restartRequired := status.Consent != nil && s.documentSearch == nil
...
func parseDocumentSearchRequest(r *http.Request) (store.DocumentSearchRequest, error) {
	...
	if candidateFound && (request.CandidateLimit < 1 || request.CandidateLimit > 1000) {
		return request, errors.New("candidate_limit must be between 1 and 1000")
	}

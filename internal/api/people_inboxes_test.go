...
func TestParticipantInboxesResolveAliasAndReturnSourceRollups(t *testing.T) {
	...
	assert.Equal(t, aliasID, engine.resolvedID)
	assert.Equal(t, query.PersonInboxRequest{CanonicalID: canonicalID}, engine.inboxRequest)

	var body query.PersonInboxResponse
	require.NoError(t, json.NewDecoder(response.Body).Decode(&body))
	require.Len(t, body.Rows, 1)
	assert.Equal(t, "cache-inboxes", body.CacheRevision)
	assert.Equal(t, int64(9), body.IdentityRevision)
	assert.Equal(t, "beeper", body.Rows[0].SourceType)
	assert.Equal(t, "whatsapp", body.Rows[0].SourceIdentifier)
	assert.Equal(t, int64(2), body.Rows[0].ConversationCount)
	assert.Equal(t, int64(3), body.Rows[0].ReceivedCount)
	assert.Equal(t, int64(4), body.Rows[0].SentCount)
	assert.Equal(t, &receivedAt, body.Rows[0].LatestReceivedAt)
	assert.Equal(t, &sentAt, body.Rows[0].LatestSentAt)
	assert.Equal(t, sentAt, body.Rows[0].LatestAt)
}

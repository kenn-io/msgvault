import {
  listPersonFactClaims as generatedListPersonFactClaims,
  listPersonFactDecisions as generatedListPersonFactDecisions,
  listPersonFactEvidence as generatedListPersonFactEvidence,
  listPersonFactEvidenceStatusEvents as generatedListPersonFactEvidenceStatusEvents,
  listPersonFactPins as generatedListPersonFactPins,
  listPersonFactTargets as generatedListPersonFactTargets,
} from '../api/generated/api/api';
import { SvelteSet } from 'svelte/reactivity';
import type { APIClient } from '../api/client';
import type {
  PersonFactClaim as GeneratedPersonFactClaim,
  PersonFactDecision as GeneratedPersonFactDecision,
  PersonFactEvidence as GeneratedPersonFactEvidence,
...
type RawEvidence = GeneratedPersonFactEvidence;
type RawClaim = GeneratedPersonFactClaim;
type RawDecision = GeneratedPersonFactDecision;
...
export class FactLedgerController {
  ...
  get initialLoading(): boolean {
    return (
      this.catalogLoading || this.evidence.loading || this.claims.loading || this.decisions.loading || this.pins.loading
    );
  }
  ...
  hasPrevious(section: PagedSection): boolean {
    return this[section].offset > 0;
  }
  ...
  get hasHistoryNext(): boolean {
    return !this.history.endReached && this.history.rows.length === FACT_LEDGER_PAGE_LIMIT;
  }
  ...
  private startLane(name: keyof typeof this.lanes): {
    abort: AbortController;
    generation: number;
  } {
    this.abortLane(name);
  ...
  private async loadCatalog(context: number): Promise<void> {
    ...
    this.catalogError = null;
    try {
      const response = await generatedListPersonFactTargets(
      ...
      for (const raw of response.data.targets ?? []) {
        ...
        if (
          ...
          continue;
        targets.push({
          optionID: `target-${targets.length}`,
  ...
  private async loadPins(context: number): Promise<void> {
    ...
    try {
      const response = await generatedListPersonFactPins(
        { id: personID },
        {
          ...this.client,
          signal: abort.signal,
      ...
      if (!response.data) {
        this.pins.rows = [];
        this.pins.error = 'Unable to load fact pins.';
        return;
      }
      this.pins.rows = (response.data.pins ?? []).map((pin, index) => {
        ...
        return {
          ...
          description: descriptor?.description ?? 'Eligible fact target',
  ...
  private async loadPage(section: PagedSection, targetOffset: number, context: number): Promise<void> {
    ...
    const query = {
      ...
      limit: FACT_LEDGER_PAGE_LIMIT,
      offset: targetOffset,
    };
    try {
      const response =
        section === 'evidence'
          ? await generatedListPersonFactEvidence({ id: personID }, query, {
              ...this.client,
              signal: abort.signal,
            })
          : section === 'claims'
            ? await generatedListPersonFactClaims({ id: personID }, query, {
                ...this.client,
                signal: abort.signal,
              })
            : await generatedListPersonFactDecisions({ id: personID }, query, {
                ...this.client,
                signal: abort.signal,
              });
      if (!this.owns(section, abort, generation, context)) return;
      if (!response.data) {
        this.failPage(section, targetOffset);
        return;
      }
      const rawRows =
        section === 'evidence'
      ...
      const projected =
        section === 'evidence'
          ? (rawRows as RawEvidence[]).map((row, index) => projectEvidence(row, targetOffset + index))
          : section === 'claims'
            ? (rawRows as RawClaim[]).map((row, index) => projectClaim(row, targetOffset + index))
  ...
  private async loadHistory(targetOffset: number, context: number): Promise<void> {
    ...
    page.retryOffset = undefined;
    try {
      const response = await generatedListPersonFactEvidenceStatusEvents(
        { id: personID },
        { evidence_key: evidenceKey, limit: FACT_LEDGER_PAGE_LIMIT, offset: targetOffset },
        {
          ...this.client,
          signal: abort.signal,
        },
      ...
      if (
        !this.owns('history', abort, generation, context) ||
        !this.historyOpen ||
        this.historyEvidenceKey !== evidenceKey
      )
        return;
      if (!response.data) {
        this.failHistory(targetOffset);
        return;
      }
      const rows = response.data.events.map((event) => ({
        supported: event.supported,
        reasonLabel: evidenceStatusReasonLabel(event.reason),
        createdAt: event.created_at,
      }));
      if (targetOffset > page.offset && rows.length === 0 && page.rows.length > 0) {
        page.endReached = true;
        return;
      }
      page.rows = rows;
  ...
  private failHistory(targetOffset: number): void {
    if (this.history.rows.length === 0 && this.history.offset === 0 && targetOffset === 0)
      this.history.error = 'Unable to load support history.';
    else {
      this.history.pageError = 'Unable to load the requested support-history page.';
      this.history.retryOffset = targetOffset;
    }
  }
}
function projectEvidence(row: RawEvidence, index: number): FactEvidenceRow {
  return {
    rowToken: `evidence-${index}`,
...
function projectClaim(row: RawClaim, index: number): FactClaimRow {
  return {
    ...
    reportedScore: row.confidence.reported_score,
    createdAt: row.created_at,
    targetCanonical: encodeFactTargetRef(row.target),
  };
}
function projectDecision(row: RawDecision, index: number): FactDecisionRow {
  return {
    rowToken: `decision-${index}`,
    action: row.action,
    reason: row.reason,
    score: {
      authority: row.score.authority,
      confidence: row.score.confidence,
      corroboration: row.score.corroboration,
      directness: row.score.directness,
      freshness: row.score.freshness,
      sourceClass: row.score.source_class,
      total: row.score.total,
    },
    projectionKind: row.projection?.kind ?? null,
    createdAt: row.created_at,
  };
}

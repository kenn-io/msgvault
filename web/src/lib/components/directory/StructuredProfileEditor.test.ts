...
type PersonContactPoint = GeneratedPersonContactPoint;
...
function bundle(): DirectoryReadBundle {
  return {
    ...
    etags: { person: '"person-7-r3"', structuredProfile: '"person-7-r3"' },
    errors: {},
  };
}
...
function requestHarness(
  response: () => Response = () =>
    new Response(JSON.stringify(profile(4)), {
...
describe('StructuredProfileEditor', () => {
  it('supersedes a contact point with generated user provenance and no fabricated actor', async () => {
    ...
    const current = {
      person_id: 7,
      address_kind: 'email',
      original_value: 'old@example.test',
    ...
    await expect(requests[0]!.clone().json()).resolves.toEqual({
      contact_points: {
        add: [
          {
            address_kind: 'email',
            original_value: 'alice@example.test',
      ...
      expected: {
        dates: {
          add: [
            {
              date_kind: 'birthday',
              original_value: '2000-02-03',
              date_text: '2000-02-03',
              date: { year: 2000, month: 2, day: 3 },
              envelope: { source: 'user' },
            },
          ],
          supersede: [],
          ...
          add: [
            {
              ...
              original_value: 'https://example.test/avatar.png',
              envelope: { source: 'user' },
            },
          ],
          supersede: [],
  ...
  it('edits an address represented by free text and geo without inventing a street', async () => {
    ...
    const current = {
      person_id: 7,
      address_kind: 'birth_place',
      original_value: 'Original imported place',
      free_text: 'Exampleville, Testland',
      geo_uri: 'geo:1,2',
        ...
        add: [
          {
            ...
            geo_uri: 'geo:1,2',
            country_name: 'Testland',
            envelope: { source: 'user', ordinal: 8 },
          },
        ],
        supersede: [26],
      },
  ...
  it.each([
    {
      section: 'names' as const,
      label: 'Formatted name',
      value: 'Dr. Alice Example',
      save: 'Save name',
      current: {
        person_id: 7,
        name_kind: 'structured',
        original_value: 'Alice Example',
        formatted: 'Alice Example',
        family_name: 'Example',
        given_name: 'Alice',
        additional_names: 'Beatrice',
        honorific_prefixes: 'Dr.',
        honorific_suffixes: 'PhD',
        secondary_surname: 'Sample',
        generation: 'Jr.',
        language: 'en',
        script: 'Latn',
        phonetic_system: 'ipa',
        phonetic_script: 'Latn',
        sort_as: 'Example, Alice',
        is_derived: false,
      ...
      expected: {
        names: {
          add: [
            {
              name_kind: 'structured',
              original_value: 'Dr. Alice Example',
              formatted: 'Dr. Alice Example',
              family_name: 'Example',
              given_name: 'Alice',
              additional_names: 'Beatrice',
              honorific_prefixes: 'Dr.',
    ...
    {
      section: 'contact_points' as const,
      label: 'Contact value',
      value: 'alice.updated',
      save: 'Save contact point',
      current: {
        person_id: 7,
        address_kind: 'username',
        ...
        scope_value: 'T-SYNTHETIC',
        uri: 'https://example.test/alice',
      ...
      expected: {
        contact_points: {
          add: [
            {
              address_kind: 'username',
              original_value: 'alice.updated',
              service_slug: 'slack',
              scope_kind: 'workspace',
              scope_value: 'T-SYNTHETIC',
              uri: 'https://example.test/alice',
              envelope: { source: 'user', ordinal: 4 },
            },
          ],
          supersede: [22],
        },
    ...
    },
    {
      section: 'addresses' as const,
      label: 'Street address',
      value: '2 Updated Street',
      save: 'Save address',
      current: {
        person_id: 7,
        address_kind: 'postal',
        original_value: 'original-vcard-value',
        post_office_box: 'PO 12',
        ...
        postal_code: '12345',
        country_name: 'Testland',
        extended_components: 'Building A',
        free_text: 'Deliver after noon',
        label: 'Home',
        geo_uri: 'geo:1,2',
        timezone: 'Etc/UTC',
        country_code: 'TS',
        place_uri: 'https://example.test/place',
      ...
      expected: {
        addresses: {
          add: [
            {
              address_kind: 'postal',
              original_value: '2 Updated Street',
              post_office_box: 'PO 12',
              extended_address: 'Suite 3',
              street_address: '2 Updated Street',
              locality: 'Testville',
              region: 'TS',
              postal_code: '12345',
              country_name: 'Testland',
    ...
    },
    {
      section: 'media' as const,
      label: 'Media URI',
      value: 'https://example.test/new-avatar.png',
      save: 'Save media metadata',
      current: {
        person_id: 7,
        media_kind: 'photo',
        original_value: 'Profile portrait',
        uri: 'https://example.test/avatar.png',
          ...
          add: [
            {
              media_kind: 'photo',
              original_value: 'Profile portrait',
              uri: 'https://example.test/new-avatar.png',
              media_type: 'image/png',
    ...
    'preserves untouched generated fields when replacing $section',
    async ({ section, label, value, save, current, expected }) => {
      const { controller, requests } = requestHarness();
  ...
  it.each([
    {
      ...
      current: {
        ...
        formatted: 'Alice',
        is_derived: false,
        envelope: { id: 31, ordinal: 2, source: 'user', created_at: when, updated_at: when, vcard: {} },
      },
    },
    {
      section: 'contact_points' as const,
      selector: 'Contact kind',
      nextKind: 'phone',
      save: 'Save contact point',
      family: 'contact_points',
      current: {
        person_id: 7,
        address_kind: 'email',
        original_value: 'alice@example.test',
        ...
        normalization: 'email',
        normalization_version: 1,
        envelope: { id: 32, ordinal: 2, source: 'user', created_at: when, updated_at: when, vcard: {} },
      },
    },
    {
      section: 'addresses' as const,
      selector: 'Address kind',
      nextKind: 'birth_place',
      save: 'Save address',
      family: 'addresses',
      current: {
        person_id: 7,
        ...
        original_value: '1 Test Street',
        street_address: '1 Test Street',
        envelope: { id: 33, ordinal: 2, source: 'user', created_at: when, updated_at: when, vcard: {} },
      },
    },
    {
      section: 'dates' as const,
      selector: 'Date kind',
      nextKind: 'anniversary',
      save: 'Save date',
      family: 'dates',
      current: {
        person_id: 7,
        ...
        date: { year: 2000 },
        date_text: '2000',
        envelope: { id: 34, ordinal: 2, source: 'user', created_at: when, updated_at: when, vcard: {} },
      },
    },
    {
      section: 'media' as const,
      ...
      current: {
        person_id: 7,
        media_kind: 'photo',
        original_value: 'https://example.test/photo.png',
        uri: 'https://example.test/photo.png',
        has_data: false,
        envelope: { id: 35, ordinal: 2, source: 'user', created_at: when, updated_at: when, vcard: {} },
      },
    },
  ])(
    'omits the prior ordinal when replacing $section with a different discriminator',
    async ({ section, selector, nextKind, save, family, current }) => {
      const { controller, requests } = requestHarness();
      render(StructuredProfileEditor, { controller, section, current });
      ...
      const body = (await requests[0]!.clone().json()) as Record<
        string,
        { add: Array<{ envelope: Record<string, unknown> }>; supersede: number[] }
      >;
  ...
  it.each([
    ...
    const current = {
      person_id: 7,
      date_kind: 'birthday',
      original_value: 'server-original',
      date,
      date_text: 'server-display',
  ...
  it('keeps Save disabled through a stale reload and restores it only after a fresh strong profile ETag', async () => {
    ...
    const fetchFn = vi.fn<typeof fetch>(async (input) => {
      ...
      if (request.method === 'PATCH')
        return Response.json({ error: 'precondition_required', message: 'Reload this profile.' }, { status: 428 });
      if (path === '/api/v1/people/7')
        return new Response(JSON.stringify(person(4)), { headers: { ETag: '"person-7-r4"' } });
      if (path.endsWith('/profile')) {
        profileReloads += 1;
        return new Response(JSON.stringify(profile(4)), {
          headers: { ETag: profileReloads === 1 ? '"person-7-r3"' : '"person-7-r4"' },
        });
      }
    ...
    expect(screen.getByRole('button', { name: 'Save category' })).toHaveProperty('disabled', true);
    await waitFor(() =>
      expect(screen.getByRole('button', { name: 'Reload profile' })).toHaveProperty('disabled', false),
    );
    ...
    await waitFor(() =>
      expect(screen.getByRole('button', { name: 'Save category' })).toHaveProperty('disabled', false),
    );
  });
      ...
      Response.json(
        {
          ...
          message: 'That category is already current.',
        },
        { status: 409 },
      ),
    );
  ...
  it('refuses a direct unsafe edit of inline media whose bytes are not in the read contract', () => {
    ...
    const current = {
      person_id: 7,
      media_kind: 'photo',
      original_value: 'Inline portrait',
      has_data: true,
      media_type: 'image/png',
      byte_size: 128,
      content_hash: 'sha256:synthetic',
    ...
    expect(screen.getByRole('note').textContent).toContain(
      'Inline media content cannot be replaced from metadata alone.',
    );

<script lang="ts">
  ...
  import type {
    PersonProfileHistory as GeneratedPersonProfileHistory,
    ValueEnvelope as GeneratedValueEnvelope,
  } from '../../api/generated/models';
  type PersonProfileHistory = GeneratedPersonProfileHistory;
  ...
  async function load(): Promise<void> {
    ...
    error = null;
    try {
      const { data, error: responseError, response } = await generatedGetPersonProfileHistory({ id: personID }, client);
      if (data) {
        history = data;
        return;
      }
...
<Modal
  ...
  ariaLabel="Profile history"
  closeLabel="Close profile history"
  onclose={onClose}
      ...
      <section>
        ...
        <ul>
          {#each (history.contact_points ?? []).filter((row) => historical(row.envelope)) as row (row.envelope.id)}
            <li>
              <strong>{row.original_value}</strong>{#if providerContext(row)}<small>{providerContext(row)}</small
                >{/if}<small>{provenance(row.envelope)}</small>
            </li>
      ...
      <section>
        ...
        <ul>
          {#each (history.dates ?? []).filter((row) => historical(row.envelope)) as row (row.envelope.id)}
            <li>
              <strong>{row.label ?? row.date_kind}: {row.date_text ?? row.original_value}</strong><small
                >{provenance(row.envelope)}</small
              >
            </li>
      ...
      <section>
        ...
        <ul>
          {#each history.observations ?? [] as row (row.envelope.id)}
            <li>
              <strong>{row.original_value}</strong>{#if providerContext(row)}<small>{providerContext(row)}</small
                >{/if}<small>{provenance(row.envelope)}{row.observed_at ? ` · Observed: ${row.observed_at}` : ''}</small
              >
            </li>
...
<style>
  .profile-history {
    display: grid;
    gap: var(--space-4);
    min-width: min(42rem, calc(100vw - 64px));
  }
  section,
  li {
    display: grid;

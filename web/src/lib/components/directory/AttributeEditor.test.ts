...
function attributeWrite(body: GeneratedSetPersonAttributeRequest, definitionValue: AttributeDefinition, id = 90) {
  return {
    dry_run: false,
    value: personValue(definitionValue, id, body.value, body.ordinal ?? 0),
    ...(body.expected_value_id
      ? {
          superseded: personValue(
            definitionValue,
            body.expected_value_id,
            { type: definitionValue.value_type, text: 'Old value' },
            body.ordinal ?? 0,
            { active_until: when, superseded_at: when },
          ),
        }
      : {}),
  };
...
describe('AttributeEditor', () => {
  ...
  it.each([
    ...
    },
    {
      name: 'integer',
      definition: definition({
        slug: 'contact_frequency',
        universal_id: 'integer-id',
    ...
    },
    {
      ...
      definition: definition({
        slug: 'introduced_by',
        universal_id: 'person-id',
        label: 'Introduced by',
        value_type: 'record_reference',
        field_type: 'person',
        record_target: 'person',
        options: undefined,
      }),
    ...
    },
    {
      ...
      input: async () =>
        fireEvent.input(screen.getByLabelText('Nickname'), { target: { value: 'Synthetic nickname' } }),
      want: { type: 'text', text: 'Synthetic nickname' },
    },
  ])(
    'builds the generated $name value union without CAS for a new value',
    async ({ definition: definitionValue, input, want }) => {
      const requests: Request[] = [];
  ...
  it('replaces the selected multi-value lineage with its exact value ID and ordinal', async () => {
    ...
    await waitFor(() => expect(requests).toHaveLength(1));
    await expect(requests[0]!.clone().json()).resolves.toEqual({
      value: { type: 'text', text: 'New topic' },
      expected_value_id: 33,
      ordinal: 4,
      source: 'user',
    });
  });
  ...
  it.each([
    ...
    'emits the exact generated $name union for a valid definition-driven draft',
    async ({ definition: definitionValue, draft, want }) => {
      const requests: Request[] = [];
      const fetchFn = vi.fn<typeof fetch>(async (input) => {
        const request = input instanceof Request ? input : new Request(input);
        requests.push(request);
  ...
  it.each([
    ...
    [
      ...
      '2025-02-29T14:30:45Z',
      'Enter an RFC3339 timestamp.',
    ],
    [
      'normalized 24-hour time',
      definition({
        slug: 'follow_up_at',
        label: 'Follow up at',
        value_type: 'timestamp',
  ...
  it('rebases a conflicted multi edit to the fresh current ID at the same ordinal', async () => {
    ...
    const fetchFn = vi.fn<typeof fetch>(async (input) => {
      ...
      if (url.pathname.endsWith('/attributes'))
        return Response.json({
          person_id: 7,
          attributes: [{ definition: note, current: [server], history: [server] }],
        });
      if (url.pathname === '/api/v1/attribute-definitions') return Response.json({ definitions: [note] });
      if (url.pathname.endsWith('/profile'))
        return new Response(JSON.stringify({ person: { id: 7 } }), {
    ...
    await waitFor(() =>
      expect(screen.getByRole('button', { name: 'Save attribute' })).toHaveProperty('disabled', false),
    );
    expect(screen.getByLabelText('Note')).toHaveProperty('value', 'Local draft');
    ...
    await expect(setRequests[1]!.clone().json()).resolves.toEqual({
      value: { type: 'text', text: 'Local draft' },
      expected_value_id: 21,
      ordinal: 4,
      source: 'user',
    });
  });
  ...
  it('requires deliberate conversion to a new add when a conflicted multi lineage disappeared', async () => {
          ...
          attributes: [
            {
              ...
              current: [],
              history: [
                personValue(note, 19, { type: 'text', text: 'Original' }, 4, {
                  active_until: when,
                  superseded_at: when,
                }),
              ],
    ...
    expect(
      await screen.findByText('The selected value is no longer current. Cancel or add this draft as a new value.'),
    ).toBeDefined();
    expect(screen.getByLabelText('Note')).toHaveProperty('value', 'Local draft');
    expect(screen.getByRole('button', { name: 'Save attribute' })).toHaveProperty('disabled', true);

<script lang="ts">
  ...
  import type {
    ...
    ValueEnvelope as GeneratedValueEnvelope,
  ...
  } from './StructuredProfileEditor.svelte';
  type ValueEnvelope = GeneratedValueEnvelope;
  type PersonContactPoint = GeneratedPersonContactPoint;
  type ParticipantContactObservation = GeneratedParticipantContactObservation;
  interface Props {
    client: APIClient;
  ...
  $effect(() => {
    const signature = (controller.structuredProfile?.contact_points ?? [])
      .map((point) =>
        [
          point.address_kind,
  ...
  function rows(section: StructuredProfileSectionName): StructuredProfileRecord[] {
    ...
    if (section === 'contact_points') {
      result.sort(
        (left, right) =>
          contactService(left).localeCompare(contactService(right)) ||
          value(section, left).localeCompare(value(section, right)),
      );
    }
    return result;
  }
  ...
  function sameObservation(point: PersonContactPoint, observation: ParticipantContactObservation): boolean {
    return (
      point.address_kind === observation.address_kind &&
  ...
  function backingObservation(record: StructuredProfileRecord): ParticipantContactObservation | undefined {
    ...
    return observations
      ...
      .sort((left, right) =>
        (right.observed_at ?? right.envelope.updated_at).localeCompare(left.observed_at ?? left.envelope.updated_at),
      )[0];
  }
  ...
  async function loadObservations(signal: AbortSignal): Promise<void> {
    try {
      const response = await generatedGetPersonProfileHistory(
        { id: personID },
        {
          ...client,
          signal,
  ...
  function providerContext(record: StructuredProfileRecord): string | null {
    const point = record as GeneratedPersonContactPoint;
    const scope =
      point.scope_kind && point.scope_value
        ? `${point.scope_kind} / ${point.scope_value}`
        : (point.scope_kind ?? point.scope_value);
    const details = [
      point.service_slug ? `Service: ${point.service_slug}` : undefined,
  ...
  function isInlineMedia(section: StructuredProfileSectionName, record: StructuredProfileRecord): boolean {
    ...
    const media = record as GeneratedPersonMedia;
  ...
  function closePatch(section: StructuredProfileSectionName, id: number): PersonProfilePatchRequest {
    switch (section) {
      case 'names':
        return { names: { supersede: [id] } };
      case 'contact_points':
        return { contact_points: { supersede: [id] } };
      case 'addresses':
        return { addresses: { supersede: [id] } };
      case 'dates':
        return { dates: { supersede: [id] } };
      case 'categories':
        return { categories: { supersede: [id] } };
      case 'media':
        return { media: { supersede: [id] } };
    }
  }
...
<section class="structured-profile" aria-label="Structured profile">
  ...
  {#each sections as descriptor (descriptor.section)}
    <section class="profile-group">
          ...
          <li>
            <div class="record-copy">
              ...
              {#if descriptor.section === 'contact_points' && providerContext(record)}<small
                ...
                >{/if}
              {#if descriptor.section === 'contact_points' && observationText(record)}<small
                  >{observationText(record)}</small
                >{/if}
              {#if isInlineMedia(descriptor.section, record)}<small
                  >Inline content is read-only here because metadata editing cannot preserve its stored bytes.</small
                >{/if}
            ...
            <div class="record-actions">
              ...
              {/if}
              <Button
                label={`Close ${descriptor.singular} ${actionValue(descriptor.section, record)}`}
                shortLabel="Close"
                size="sm"
                tone="danger"
                disabled={!controller.canWriteProfile}
                onclick={() => {
                  confirming = { section: descriptor.section, current: record };
                  editing = undefined;
                }}
              />
            </div>
            {#if confirming?.section === descriptor.section && confirming.current.envelope.id === record.envelope.id}
              ...
              >
                ...
                />
                <Button
                  label={`Confirm close ${descriptor.singular}`}
                  size="sm"
                  tone="danger"
                  surface="solid"
                  disabled={!controller.canWriteProfile}
      ...
      {#if descriptor.section === 'contact_points' && observationsError}<p class="observation-error" role="status">
          {observationsError}
        </p>{/if}

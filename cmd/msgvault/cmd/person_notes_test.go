...
const testPersonNotesAttributesJSON = `{
	...
	"attributes":[{
		"definition":{
			"id":9,"universal_id":"field-notes","object_type":"person","slug":"notes","label":"Notes",
...
func TestPersonNotesGetRoutesThroughDaemonAndPrintsOnlyText(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "notes", r.URL.Query().Get("slug"))
...
func TestPersonNotesGetJSONEmitsFullCurrentValue(t *testing.T) {
	...
	assert.Equal(t, float64(71), value["id"])
	assert.Equal(t, "notes", value["definition_slug"])
	assert.Equal(t, "cli", value["actor"])
	assert.Equal(t, map[string]any{"type": "text", "text": "Private\ncontext"}, value["value"])
}
...
func TestPersonNotesSetForwardsTypedTextAndOptionalCAS(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "/api/v1/people/7/attributes/notes", r.URL.Path)
	...
	require.NoError(t, err)
	assert.Equal(t, map[string]any{"text": "Replacement", "type": "text"}, body["value"])
	assert.Equal(t, float64(71), body["expected_value_id"])
	assert.Equal(t, "user", body["source"])
	var write map[string]any
	require.NoError(t, json.Unmarshal([]byte(output), &write))
	assert.Equal(t, false, write["dry_run"])
	assert.Equal(t, float64(72), write["value"].(map[string]any)["id"])
	assert.Equal(t, float64(71), write["superseded"].(map[string]any)["id"])
}
...
func TestPersonNotesSetReadsFileWithoutDiscardingMultilineText(t *testing.T) {
	...
	assert.NotContains(t, body, "expected_value_id")
}

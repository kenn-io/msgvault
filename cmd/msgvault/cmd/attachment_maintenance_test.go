...
func TestPendingPackContinuesAfterByteBudget(t *testing.T) {
	for _, daily := range []bool{false, true} {
		t.Run(fmt.Sprintf("daily=%v", daily), func(t *testing.T) {
			...
			run := f.maintenance.runPendingPack
			if daily {
				run = f.maintenance.daily
			}
			require.ErrorIs(t, run(context.Background()), scheduler.ErrReschedule,
				"a bounded pass must request a follow-up for the remaining backlog")
			require.NoError(t, run(context.Background()))
			for _, hash := range hashes {
				assert.NotNil(t, f.packedEntry(hash), "the next pass must pack the remaining blobs")
			}
		})
	}
}

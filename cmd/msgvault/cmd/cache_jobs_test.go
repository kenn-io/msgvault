...
func TestCacheBuildJobsSyncDuringBuildGetsFollowup(t *testing.T) {
	...
	assert.Equal(buildCacheModeAuto, <-started,
		"post-write follow-up must bypass the interval reset by the first build")
	status, ok := jobs.status(first.JobID)
	require.True(ok)
	assert.Equal(api.CacheBuildRunning, status.Status,
		"accepted job must remain active until the post-sync follow-up publishes")
	assert.Nil(status.FinishedAt)
	close(releaseSecond)
...
func TestCacheBuildJobsFollowupFailureFailsAcceptedJob(t *testing.T) {
	require := require.New(t)
	...
	jobs := newCacheBuildJobs(t.Context(), nil, func(context.Context, buildCacheMode) error {
		...
		return errors.New("synthetic follow-up failure")
	})
	first, err := jobs.accept(buildCacheModeAuto)
	require.NoError(err)
	<-started
	_, err = jobs.acceptAfterWrite(buildCacheModeScheduledAuto)
	require.NoError(err)
	close(releaseFirst)
	require.Eventually(func() bool {
		status, ok := jobs.status(first.JobID)
	...
	status, ok := jobs.status(first.JobID)
	require.True(ok)
	require.NotNil(status.FinishedAt)

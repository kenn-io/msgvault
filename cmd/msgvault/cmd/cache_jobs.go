...
func (m *cacheBuildJobs) execute(id string, mode buildCacheMode, done func()) {
	...
	for {
		...
		if m.pending && m.ctx.Err() == nil {
			// A sync committed while this build was reading an older snapshot.
			// Keep the accepted job active until its follow-up publishes those rows.
			if err != nil {
				logger.Error("background analytics cache build failed before follow-up", "job_id", id, "error", err)
			}
			mode = m.pendingMode
			if mode == buildCacheModeScheduledAuto {
				// The first build may have just reset PublishedAt. A promised
				// post-write follow-up must not be throttled by that timestamp.
		...
		if m.pending {
			// Shutdown prevented the promised follow-up; the accepted job did
			// not publish all writes, even if its earlier build succeeded.
			err = m.ctx.Err()
		}
		job = m.jobs[id]

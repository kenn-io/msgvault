...
func TestTruncateDisplayKeepsRunesWhole(t *testing.T) {
	...
	assert.Equal(t, "ки", truncateDisplay("кино", 2))
}
...
func TestWriteSearchResultsTableShowsChatSnippetAndUnknownSize(t *testing.T) {
	...
	require.NoError(t, writeSearchResultsTable(&buf, results))
	lines := strings.Split(buf.String(), "\n")
	require.GreaterOrEqual(t, len(lines), 4)
	assert.Contains(t, lines[2], "Concert tickets")
	assert.Contains(t, lines[2], "1.0K")
	assert.Contains(t, lines[3], "this is the band")
	assert.Contains(t, lines[3], "Bob Example")
	assert.NotContains(t, lines[3], "0B")
	assert.NotContains(t, buf.String(), "\x1b")
	assert.True(t, strings.HasSuffix(strings.TrimRight(lines[3], " "), " -"))
	assert.Contains(t, buf.String(), "Showing 2 results")
}

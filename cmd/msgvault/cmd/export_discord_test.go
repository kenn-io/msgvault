...
func TestExportDiscordWritesVersionedBoundedJSON(t *testing.T) {
	...
	require.NoError(err)
	require.NoError(st.CompleteSync(runID, "complete"))
	...
	assert.Equal("msgvault-discord-export/1", got.Schema)
	assert.Equal(Version, got.MsgvaultVersion)
	assert.False(got.SourceSyncCompletedAt.IsZero())
	assert.Equal(testDiscordGuildA, got.Guild.ID)
	assert.Equal("Alpha Guild", got.Guild.Name)
	assert.Equal(start, got.Start)
	assert.Equal(start.Add(24*time.Hour), got.End)
	require.Len(got.Containers, 1)
	require.Len(got.Containers[0].Messages, 1)
	assert.Equal("400000000000000001", got.Containers[0].Messages[0].ID)
}

...
func TestDeletionEscalationScopes_ReadonlyWorld(t *testing.T) {
	tests := []struct {
		...
		{
			// Full access already permits trashing, so requesting modify on
			// top of it would widen the grant for nothing.
			name:        "trash escalation does not add modify to a full access grant",
			batchDelete: false,
			existing:    []string{oauth.ScopeGmailReadonly, oauth.ScopeGmailFull},
			want:        []string{oauth.ScopeGmailReadonly, oauth.ScopeGmailFull},
		},
		{

...
func TestWriteRepackAttachmentsStats(t *testing.T) {
	...
	writeRepackAttachmentsStats(&out, repacker.Stats{
		...
		BytesRepacked: 1024, BudgetExhausted: true,
	})
	assert.Contains(t, out.String(), "Repacked 4 blob(s) (1.0K) from 2 pack(s) into 1 pack(s); removed 3 old pack(s).")
	assert.Contains(t, out.String(), "Pruned 2 stale packed blob mapping(s).")
	assert.Contains(t, out.String(), "Repack byte budget reached")
}

...
func deletionEscalationScopes(batchDelete bool, existingScopes []string) []string {
	...
	// Full access already covers trashing, so adding modify on top of it would
	// widen the request for no gain.
	if !batchDelete && slices.Contains(existingScopes, oauth.ScopeGmailFull) {

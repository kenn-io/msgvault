...
func (startupDocumentSearchLedger) GetActiveDocumentVectorGeneration(context.Context) (*store.DocumentVectorGeneration, error) {
	return &store.DocumentVectorGeneration{
		ID: 1, DocumentVectorGenerationSpec: store.DocumentVectorGenerationSpec{
...
func TestRegisterDocumentVectorJobRequiresConsentedCapability(t *testing.T) {
	assertions := assert.New(t)
	requirements := require.New(t)
	c := config.NewDefaultConfig()
	...
	withTestConfig(t, c)
	complete := &vectorFeatures{
		DocumentSearch:  vectordocument.NewSearchService(vectordocument.SearchDeps{Ledger: startupDocumentSearchLedger{}}),
	...
	requirements.NoError(registerDocumentVectorJob(capture, complete, nil))
	...
	for name, features := range map[string]*vectorFeatures{
		"disabled":     nil,
		"unconsented":  {DocumentBackend: startupDocumentBackend{}, SemanticClient: startupDocumentSemanticClient{}, Cfg: c.Vector},
		"nil backend":  {DocumentSearch: complete.DocumentSearch, SemanticClient: startupDocumentSemanticClient{}, Cfg: c.Vector},
		"nil provider": {DocumentSearch: complete.DocumentSearch, DocumentBackend: startupDocumentBackend{}, Cfg: c.Vector},
	} {

	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if response.JSON200 == nil {
				return errors.New("identity match list response was empty")
			}
			if jsonOutput {
				return writeIdentityMatchesJSON(cmd.OutOrStdout(), response.JSON200)
			}
			return writeIdentityMatchesTable(cmd.OutOrStdout(), response.JSON200.Candidates)
		},
	}
...
func newIdentityMatchesShowCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer func() { _ = client.Close() }()
			response, err := daemonclient.APIResponse(client,
				func(api *apiclient.Client) (*generated.GetIdentityMatchCandidateResp, error) {
					return api.GetIdentityMatchCandidateWithResponse(cmd.Context(),
						&generated.GetIdentityMatchCandidateRequestOptions{
							PathParams: &generated.GetIdentityMatchCandidatePath{ID: id}})
				})
			if err != nil {
				return err
			}
			if response.JSON200 == nil {
				return errors.New("identity match response was empty")
			}
			if jsonOutput {
				return writeIdentityMatchesJSON(cmd.OutOrStdout(), response.JSON200)
			}
			return writeIdentityMatchDetail(cmd.OutOrStdout(), response.JSON200)
		},
	}
	command.Flags().BoolVar(&jsonOutput, "json", false, "Print the complete API response")
	return command
...
func newIdentityMatchesDecisionCommand(action string) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if action == "accept" {
				response, err := daemonclient.APIResponse(client,
					func(api *apiclient.Client) (*generated.ReviewAcceptIdentityMatchCandidateResp, error) {
						return api.ReviewAcceptIdentityMatchCandidateWithResponse(cmd.Context(),
							&generated.ReviewAcceptIdentityMatchCandidateRequestOptions{
								PathParams: &generated.ReviewAcceptIdentityMatchCandidatePath{ID: id}, Body: body})
					})
				if err != nil {
					return err
				}
				if response.JSON200 == nil {
					return errors.New("identity match acceptance response was empty")
				}
				if jsonOutput {
					return writeIdentityMatchesJSON(cmd.OutOrStdout(), response.JSON200)
				}
				return writeIdentityMatchDecision(cmd.OutOrStdout(), &response.JSON200.Candidate,
					response.JSON200.IdentityRevision, string(response.JSON200.CacheState))
			}
			response, err := daemonclient.APIResponse(client,
				func(api *apiclient.Client) (*generated.ReviewRejectIdentityMatchCandidateResp, error) {
					return api.ReviewRejectIdentityMatchCandidateWithResponse(cmd.Context(),
						&generated.ReviewRejectIdentityMatchCandidateRequestOptions{
							PathParams: &generated.ReviewRejectIdentityMatchCandidatePath{ID: id}, Body: body})
				})
			if err != nil {
				return err
			}
			if response.JSON200 == nil {
				return errors.New("identity match rejection response was empty")
			}
			if jsonOutput {
				return writeIdentityMatchesJSON(cmd.OutOrStdout(), response.JSON200)
			}

...
func TestSyncCmd_SelectedLegacyGmailRoutesThroughGmail(t *testing.T) {
	require := require.New(t)
	assert := assert.New(t)
	tmpDir := t.TempDir()
	...
	require.NoError(err, "open store")
	require.NoError(s.InitSchema(), "init schema")
	legacy, err := s.GetOrCreateSource("", "legacy@example.com")
	...
	root.SetArgs([]string{"sync", "legacy@example.com"})

	output := captureStdout(t)
	err = root.Execute()
	stdout := output()
	require.Error(err, "missing token should fail inside the Gmail path")
	assert.NotContains(err.Error(), "cannot be synced")
	assert.Contains(stdout, "add-account", "legacy source should reach Gmail token handling")

	s, err = store.Open(dbPath)
	require.NoError(err, "reopen store")
	defer func() { _ = s.Close() }()
	preserved, err := s.GetSourceByID(legacy.ID)
	require.NoError(err, "reload source")
	assert.Empty(preserved.SourceType, "raw source type remains empty")
}

func TestSyncCmd_AllAccountsIncludeLegacyGmail(t *testing.T) {
	require := require.New(t)
	assert := assert.New(t)
	invalidCachedToken := `{
  ...
  "client_id": "test.apps.googleusercontent.com"
	...
	_, restore := seedTokenEnv(t, invalidCachedToken)
	defer restore()
	...
	err = root.Execute()
	require.Error(err, "the cached token should fail inside the Gmail path")
	assert.NotContains(err.Error(), "no accounts are ready to sync")

...
- Add restricted agent grants: `agent-token issue/list/revoke` (owner-only) issue in-memory, scoped tokens with 24-hour default lifetimes that are invalidated on daemon restart. A delegated caller presenting `--agent-url` and `--agent-token-file` can run only `draft-reply` and `show-message`, restricted to sources named in its grant. Enable with `[server] agent_access = true` alongside a non-empty `api_key`. See [agent-token](cli-reference.md#agent-token) and [configuration](configuration.md#server).

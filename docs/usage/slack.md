...
| Flag | Description |
|---|---|
| `--limit N` | Max messages of work per conversation this run — thread replies count via their `reply_count` forecast. Limited runs resume next run (large threads drain across runs) and skip the maintenance rescan |

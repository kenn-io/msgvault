...
- `--loose-attachments` disables direct pack installation and materializes every attachment as an individual file. On an overwritten target, pre-existing pack files can still be discovered by later maintenance. For a guaranteed fully loose archive, use a fresh target or run `unpack-attachments` after restore.

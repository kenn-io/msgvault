...
4. When `--agent-url` and `--agent-token-file` are both supplied, the CLI connects to that remote daemon as a restricted delegated caller using the token from the file. Only `draft-reply` is currently executable in this mode; `draft-get`, `draft-edit`, and `draft-delete` are recognized but remain closed until their handlers carry grants and source checks. Grants can express four independent draft permissions: `draft.create`, `draft.read`, `draft.edit`, and `draft.delete`. Sending is a separate permission and workflow not yet available. Owner configuration (`--config`, `--home`, `--local`) is rejected, and the token is never written to logs or argv. The token is transmitted in the `X-Msgvault-Agent-Token` request header; this header is not modeled in the generated OpenAPI clients — it is a transport detail that the CLI handles internally.
...
| `--permissions <perms>` | Comma-separated permission names to grant; accepted values: `draft.create`, `draft.read`, `draft.edit`, `draft.delete` |

...
4. When `--agent-url` and `--agent-token-file` are both supplied, the CLI connects to that remote daemon as a restricted delegated caller using the token from the file. The grant vocabulary includes `draft.create`, `draft.read`, `draft.edit`, and `draft.delete`; only `draft-reply` currently executes, and sending is separate. Owner configuration (`--config`, `--home`, `--local`) is rejected, and the token is never written to logs or argv. The token is transmitted in the `X-Msgvault-Agent-Token` request header; this header is not modeled in the generated OpenAPI clients — it is a transport detail that the CLI handles internally.
...
| `--permissions <perms>` | Comma-separated permission names to grant; accepted values: `draft.create`, `draft.read`, `draft.edit`, `draft.delete`. Only `draft-reply` currently executes through delegated mode; sending is separate. |
